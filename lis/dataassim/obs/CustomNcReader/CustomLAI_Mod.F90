...
contains
    ...
    subroutine CustomLAI_setup(k, OBS_State, OBS_Pert_State)
        ...
        use LIS_coreMod
        use CustomNcReader_Mod, only: CustomNcReader_setup, read_CustomNetCDF
        ...
        do n=1,LIS_rc%nnest
            CustomLAI_struc(n)%varname = varname
            CustomLAI_struc(n)%min_value = min_value
            CustomLAI_struc(n)%max_value = max_value
        enddo

...
module CGLSlai_sm_Mod
    ...
    !-----------------------------------------------------------------------------
    public :: CGLSlai_sm_setup
    !-----------------------------------------------------------------------------
    ! !PUBLIC TYPES:
    !-----------------------------------------------------------------------------
    public :: CGLSlai_sm_struc
    !EOP
    type, public:: CGLSlai_sm_dec
    ...
    end type CGLSlai_sm_dec

    type(CGLSlai_sm_dec),allocatable :: CGLSlai_sm_struc(:)

contains

    !BOP
    ! 
    ! !ROUTINE: CGLSlai_sm_setup
    ! \label{CGLSlai_sm_setup}
    ! 
    ! !INTERFACE: 
    subroutine CGLSlai_sm_setup(k, OBS_State, OBS_Pert_State)
        ! !USES: 
        use ESMF
        use LIS_coreMod
        use LIS_timeMgrMod
        ...
        allocate(CGLSlai_sm_struc(LIS_rc%nnest))
        ...
        call ESMF_ConfigFindLabel(LIS_config,"CGLS LAI apply QC flags:",&
             rc=status)
        do n=1,LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config,CGLSlai_sm_struc(n)%qcflag,&
                 rc=status)
        ...
        call ESMF_ConfigFindLabel(LIS_config,"CGLS LAI is resampled:",&
             rc=status)
        do n=1,LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config,CGLSlai_sm_struc(n)%isresampled,&
                 rc=status)
        ...
        call ESMF_ConfigFindLabel(LIS_config,"CGLS LAI spatial resolution:",&
             rc=status)
        do n=1,LIS_rc%nnest
            if (CGLSlai_sm_struc(n)%isresampled) then
                call ESMF_ConfigGetAttribute(LIS_config,CGLSlai_sm_struc(n)%spatialres,&
                     rc=status)
        ...
        do n=1, LIS_rc%nnest
            if(LIS_rc%dascaloption(k).ne."none") then 
                call ESMF_ConfigGetAttribute(LIS_config,CGLSlai_sm_struc(n)%nbins, rc=status)
        ...
        do n=1,LIS_rc%nnest
            ...
            allocate(CGLSlai_sm_struc(n)%laiobs1(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
            allocate(CGLSlai_sm_struc(n)%laiobs2(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
            ...
            if(trim(LIS_rc%perturb_obs(k)).ne."none") then 
                ...
                CGLSlai_sm_struc(n)%ssdev_inp = obs_pert%ssdev(1)
        ...
        do n=1,LIS_rc%nnest
            if(LIS_rc%dascaloption(k).ne."none") then 
                ...
                call LIS_getCDFattributes(k,modelcdffile(n),&
                     CGLSlai_sm_struc(n)%ntimes, ngrid)

                allocate(ssdev(LIS_rc%obs_ngrid(k)))
                ssdev = obs_pert%ssdev(1)

                allocate(CGLSlai_sm_struc(n)%model_mu(LIS_rc%obs_ngrid(k),&
                     CGLSlai_sm_struc(n)%ntimes))
                allocate(CGLSlai_sm_struc(n)%model_sigma(LIS_rc%obs_ngrid(k),&
                     CGLSlai_sm_struc(n)%ntimes))
                allocate(CGLSlai_sm_struc(n)%obs_mu(LIS_rc%obs_ngrid(k),&
                     CGLSlai_sm_struc(n)%ntimes))
                allocate(CGLSlai_sm_struc(n)%obs_sigma(LIS_rc%obs_ngrid(k),&
                     CGLSlai_sm_struc(n)%ntimes))
                allocate(CGLSlai_sm_struc(n)%model_xrange(&
                     LIS_rc%obs_ngrid(k), CGLSlai_sm_struc(n)%ntimes, &
                     CGLSlai_sm_struc(n)%nbins))
                allocate(CGLSlai_sm_struc(n)%obs_xrange(&
                     LIS_rc%obs_ngrid(k), CGLSlai_sm_struc(n)%ntimes, &
                     CGLSlai_sm_struc(n)%nbins))
                allocate(CGLSlai_sm_struc(n)%model_cdf(&
                     LIS_rc%obs_ngrid(k), CGLSlai_sm_struc(n)%ntimes, &
                     CGLSlai_sm_struc(n)%nbins))
                allocate(CGLSlai_sm_struc(n)%obs_cdf(&
                     LIS_rc%obs_ngrid(k), CGLSlai_sm_struc(n)%ntimes, & 
                     CGLSlai_sm_struc(n)%nbins))
                ...
                call LIS_readMeanSigmaData(n,k,&
                     ...
                     modelcdffile(n), &
                     "LAI",&
                     CGLSlai_sm_struc(n)%model_mu,&
                     CGLSlai_sm_struc(n)%model_sigma)
                ...
                call LIS_readMeanSigmaData(n,k,&
                     ...
                     obscdffile(n), &
                     "LAI",&
                     CGLSlai_sm_struc(n)%obs_mu,&
                     CGLSlai_sm_struc(n)%obs_sigma)
                ...
                if(CGLSlai_sm_struc(n)%useSsdevScal.eq.1) then 
                    ...
                    do t=1,LIS_rc%obs_ngrid(k)
                        if(CGLSlai_sm_struc(n)%obs_sigma(t,jj).ne.LIS_rc%udef) then 
                            print*, ssdev(t),CGLSlai_sm_struc(n)%model_sigma(t,jj),&
                                 CGLSlai_sm_struc(n)%obs_sigma(t,jj)
                            if(CGLSlai_sm_struc(n)%obs_sigma(t,jj).ne.0) then 
        ...
        do n=1,LIS_rc%nnest
            ...
            CGLSlai_sm_struc(n)%scale = 0.033333

            if(LIS_rc%lis_obs_map_proj(k).eq."latlon") then
                if(CGLSlai_sm_struc(n)%isresampled == 1) then
                    ! images are rescaled to custom resolution
                    CGLSlai_sm_struc(n)%lat_lower_left = 90 - 0.5 * CGLSlai_sm_struc(n)%spatialres
                    CGLSlai_sm_struc(n)%lat_upper_right = -90 + 0.5 * CGLSlai_sm_struc(n)%spatialres
                    CGLSlai_sm_struc(n)%lon_lower_left = -180 + 0.5 * CGLSlai_sm_struc(n)%spatialres
                    CGLSlai_sm_struc(n)%lon_upper_right = 180 - 0.5 * CGLSlai_sm_struc(n)%spatialres
                    ! dlat is positive since LIS will figure out that latitude is
                    ! decreasing
                    CGLSlai_sm_struc(n)%dlat = CGLSlai_sm_struc(n)%spatialres
                    CGLSlai_sm_struc(n)%dlon = CGLSlai_sm_struc(n)%spatialres
                    CGLSlai_sm_struc(n)%nr = nint(180.0 / CGLSlai_sm_struc(n)%spatialres)
                    CGLSlai_sm_struc(n)%nc = 2 * CGLSlai_sm_struc(n)%nr
                else
                    ! original spatial resolution of the downloaded data product
                    CGLSlai_sm_struc(n)%lat_lower_left = 80
                    CGLSlai_sm_struc(n)%lat_upper_right = -59.9910714285396
                    CGLSlai_sm_struc(n)%lon_lower_left = -180
                    CGLSlai_sm_struc(n)%lon_upper_right = 179.991071429063
                    ! dlat is positive since LIS will figure out that latitude is
                    ! decreasing
                    CGLSlai_sm_struc(n)%dlat = 0.008928002004371148
                    CGLSlai_sm_struc(n)%dlon = 0.008928349985839856
                    CGLSlai_sm_struc(n)%nc = 40320
                    CGLSlai_sm_struc(n)%nr = 15680
                endif
            ...
            CGLSlai_sm_struc(n)%gridDesci(1) = 0  ! regular lat-lon grid
            CGLSlai_sm_struc(n)%gridDesci(2) = CGLSlai_sm_struc(n)%nc
            CGLSlai_sm_struc(n)%gridDesci(3) = CGLSlai_sm_struc(n)%nr 
            CGLSlai_sm_struc(n)%gridDesci(4) = CGLSlai_sm_struc(n)%lat_lower_left
            CGLSlai_sm_struc(n)%gridDesci(5) = CGLSlai_sm_struc(n)%lon_lower_left
            CGLSlai_sm_struc(n)%gridDesci(6) = 128
            CGLSlai_sm_struc(n)%gridDesci(7) = CGLSlai_sm_struc(n)%lat_upper_right
            CGLSlai_sm_struc(n)%gridDesci(8) = CGLSlai_sm_struc(n)%lon_upper_right
            CGLSlai_sm_struc(n)%gridDesci(9) = CGLSlai_sm_struc(n)%dlat
            CGLSlai_sm_struc(n)%gridDesci(10) = CGLSlai_sm_struc(n)%dlon
            CGLSlai_sm_struc(n)%gridDesci(20) = 64

            CGLSlai_sm_struc(n)%mi = CGLSlai_sm_struc(n)%nc*CGLSlai_sm_struc(n)%nr
            ...
            if(LIS_rc%obs_gridDesc(k,10).lt.CGLSlai_sm_struc(n)%dlon) then 
                ...
                allocate(CGLSlai_sm_struc(n)%rlat(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
                allocate(CGLSlai_sm_struc(n)%rlon(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
                allocate(CGLSlai_sm_struc(n)%n11(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
                allocate(CGLSlai_sm_struc(n)%n12(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
                allocate(CGLSlai_sm_struc(n)%n21(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
                allocate(CGLSlai_sm_struc(n)%n22(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
                allocate(CGLSlai_sm_struc(n)%w11(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
                allocate(CGLSlai_sm_struc(n)%w12(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
                allocate(CGLSlai_sm_struc(n)%w21(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
                allocate(CGLSlai_sm_struc(n)%w22(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))

                write(LIS_logunit,*)&
                     '[INFO] create interpolation input for CGLS LAI'       

                call bilinear_interp_input_withgrid(CGLSlai_sm_struc(n)%gridDesci(:), &
                     LIS_rc%obs_gridDesc(k,:),&
                     LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k),&
                     CGLSlai_sm_struc(n)%rlat, CGLSlai_sm_struc(n)%rlon,&
                     CGLSlai_sm_struc(n)%n11, CGLSlai_sm_struc(n)%n12, &
                     CGLSlai_sm_struc(n)%n21, CGLSlai_sm_struc(n)%n22, &
                     CGLSlai_sm_struc(n)%w11, CGLSlai_sm_struc(n)%w12, &
                     CGLSlai_sm_struc(n)%w21, CGLSlai_sm_struc(n)%w22)
            else

                allocate(CGLSlai_sm_struc(n)%n11(&
                     CGLSlai_sm_struc(n)%nc*CGLSlai_sm_struc(n)%nr))

                write(LIS_logunit,*)&
                     '[INFO] create upscaling input for CGLS LAI'       

                call upscaleByAveraging_input(CGLSlai_sm_struc(n)%gridDesci(:),&
                     LIS_rc%obs_gridDesc(k,:),&
                     CGLSlai_sm_struc(n)%nc*CGLSlai_sm_struc(n)%nr, &
                     LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k), CGLSlai_sm_struc(n)%n11)
            ...
            CGLSlai_sm_struc(n)%startMode = .true. 

            call ESMF_StateAdd(OBS_State(n),(/obsField(n)/),rc=status)
            call LIS_verify(status)

        enddo
    end subroutine CGLSlai_sm_setup
end module CGLSlai_sm_Mod

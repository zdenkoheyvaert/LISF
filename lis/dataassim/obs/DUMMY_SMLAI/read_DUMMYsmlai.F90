...
#include "LIS_misc.h"
!BOP
! !ROUTINE: read_DUMMYsmlai
! \label{read_DUMMYsmlai}
!
! !REVISION HISTORY:
!  
!
! !INTERFACE:
subroutine read_DUMMYsmlai(n, k, OBS_State, OBS_Pert_State)
! !USES:
   use ESMF
   use LIS_mpiMod
   use LIS_coreMod
   ...
   use DUMMYsmlai_Mod, only: DUMMYsmlai_struc
...
!EOP
   ...
   alarmCheck = LIS_isAlarmRinging(LIS_rc, "DUMMY SM+LAI read alarm")
   ...
   if (alarmCheck .or. DUMMYsmlai_struc(n)%startMode) then
      DUMMYsmlai_struc(n)%startMode = .false.
      if ( (DUMMYsmlai_struc(n)%data_designation.eq."SPL2SMP_E") .or. &
           (DUMMYsmlai_struc(n)%data_designation.eq."SPL2SMP") ) then

         DUMMYsmlai_struc(n)%smobs = LIS_rc%udef
         DUMMYsmlai_struc(n)%smtime = -1.0
         ...
         if(LIS_masterproc) then
            ...
            rc = create_filelist(trim(list_files)//char(0), &
                 "DUMMY_filelist.sm.dat"//char(0))
            if (rc .ne. 0) then
               write(LIS_logunit,*) &
                    '[WARN] Problem encountered when searching for DUMMY files'
               write(LIS_logunit,*) &
         ...
         open(ftn,file="./DUMMY_filelist.sm.dat",status='old',iostat=ierr)
         ...
         do while(ierr.eq.0)
            ...
            call read_DUMMYL2sm_data(n,k,fname,&
                 DUMMYsmlai_struc(n)%smobs,timenow)
         enddo
         call LIS_releaseUnitNumber(ftn)

      elseif (DUMMYsmlai_struc(n)%data_designation .eq. "SPL3SMP_E") then
!---------------------------------------------------------------------------
...
!---------------------------------------------------------------------------
         ...
         write (mm, '(i2.2)') LIS_rc%mo
         write (dd, '(i2.2)') LIS_rc%da
         write (CRID, '(a)') DUMMYsmlai_struc(n)%release_number

         ! EMK...Make sure only one PET calls the file system to determine what
         ! DUMMY files are available.
         if (LIS_masterproc) then

            list_files = trim(smobsdir)//'/'//trim(yyyy)//'.'//trim(mm)//'.'// &
                         trim(dd)//'/DUMMY_L3_SM_P_E_' &
                         //trim(yyyy)//trim(mm)//trim(dd)//'_'// &
                         trim(CRID)//'*.h5'
            write(LIS_logunit,*) &
                  '[INFO] Searching for ',trim(list_files)
            rc = create_filelist(trim(list_files)//char(0), &
                 "DUMMY_filelist.dat"//char(0))
            if (rc .ne. 0) then
               write(LIS_logunit,*) &
                    '[WARN] Problem encountered when searching for DUMMY files'
               write(LIS_logunit,*) &
         ...
         ftn = LIS_getNextUnitNumber()
         open (ftn, file="./DUMMY_filelist.dat", &
...
! This assumes that the 'ls command' will list the files in that order.
         ...
         do while (ierr .eq. 0)
            ...
            call read_DUMMY_E_data(n, k, 'D', fname, smobs_D)

            write (LIS_logunit, *) '[INFO] Reading ascending pass ', trim(fname)
            call read_DUMMY_E_data(n, k, 'A', fname, smobs_A)
         enddo

         DUMMYsmlai_struc(n)%smobs = LIS_rc%udef
         DUMMYsmlai_struc(n)%smtime = -1
         call LIS_releaseUnitNumber(ftn)
!-------------------------------------------------------------------------
...
!-------------------------------------------------------------------------
                  if (smobs_A(c + (r - 1)*LIS_rc%obs_lnc(k)) .ne. -9999.0 .and. &
                      DUMMYsmlai_struc(n)%smobs(c, r) .eq. -9999.0) then
                     DUMMYsmlai_struc(n)%smobs(c, r) = &
      ...
      elseif (DUMMYsmlai_struc(n)%data_designation .eq. "SPL3SMP") then
!-------------------------------------------------------------------------
! MN: create filename for 36km product  (DUMMY_L3_SM_P_)
!-------------------------------------------------------------------------
...
! This assumes that the 'ls command' will list the files in that order.
         ...
         do while (ierr .eq. 0)
            ...
            write (LIS_logunit, *) '[INFO] Reading descending pass ', trim(fname)
            call read_DUMMY_data(n, k, 'D', fname, smobs_D)

            write (LIS_logunit, *) '[INFO] Reading ascending pass ', trim(fname)
            call read_DUMMY_data(n, k, 'A', fname, smobs_A)
         enddo
      ...
      do r=1,LIS_rc%obs_lnr(k)
         do c=1,LIS_rc%obs_lnc(k)
            if(LIS_obs_domain(n,k)%gindex(c,r).ne.-1) then
               ...
               dt = (DUMMYsmlai_struc(n)%smtime(c,r)-time1)
      ...
      do r = 1, LIS_rc%obs_lnr(k)
         do c = 1, LIS_rc%obs_lnc(k)
            if (LIS_obs_domain(n, k)%gindex(c, r) .ne. -1) then
               ...
               dt = (LIS_rc%gmt - DUMMYsmlai_struc(n)%smtime(c, r))*3600.0
...
!-------------------------------------------------------------------------
   ...
   if (DUMMYsmlai_struc(n)%ntimes .gt. 1 .and. DUMMYsmlai_struc(n)%cdf_read_opt .eq. 1) then
      if (.not. DUMMYsmlai_struc(n)%cdf_read_mon .or. LIS_rc%da .eq. 1 .and. LIS_rc%hr .eq. 0 .and. &
          LIS_rc%mn .eq. 0 .and. LIS_rc%ss .eq. 0) then
         call LIS_readMeanSigmaData(n, k, &
                                    DUMMYsmlai_struc(n)%ntimes, &
                                    LIS_rc%obs_ngrid(k), &
                                    DUMMYsmlai_struc(n)%modelcdffile, &
                                    "SoilMoist", &
                                    DUMMYsmlai_struc(n)%model_mu, &
                                    DUMMYsmlai_struc(n)%model_sigma, &
                                    LIS_rc%mo)

         call LIS_readMeanSigmaData(n, k, &
                                    DUMMYsmlai_struc(n)%ntimes, &
                                    LIS_rc%obs_ngrid(k), &
                                    DUMMYsmlai_struc(n)%obscdffile, &
                                    "SoilMoist", &
                                    DUMMYsmlai_struc(n)%obs_mu, &
                                    DUMMYsmlai_struc(n)%obs_sigma, &
                                    LIS_rc%mo)

         call LIS_readCDFdata(n, k, &
                              DUMMYsmlai_struc(n)%nbins, &
                              DUMMYsmlai_struc(n)%ntimes, &
                              LIS_rc%obs_ngrid(k), &
                              DUMMYsmlai_struc(n)%modelcdffile, &
                              "SoilMoist", &
                              DUMMYsmlai_struc(n)%model_xrange, &
                              DUMMYsmlai_struc(n)%model_cdf, &
                              LIS_rc%mo)
         ...
         call LIS_readCDFdata(n, k, &
                              ...
                              DUMMYsmlai_struc(n)%obscdffile, &
                              "SoilMoist", &
                              DUMMYsmlai_struc(n)%obs_xrange, &
                              DUMMYsmlai_struc(n)%obs_cdf, &
                              LIS_rc%mo)

         DUMMYsmlai_struc(n)%cdf_read_mon = .true.
      endif
   endif
   ...
   call lsmdaqcobsstate(trim(LIS_rc%lsm)//"+" &
                        //trim(LIS_DUMMYsmlaiobsID)//char(0), n, k, OBS_state)
   ...
   if (data_upd) then
      ...
      if (LIS_rc%dascaloption(k) .eq. "CDF matching") then
         if (DUMMYsmlai_struc(n)%useSsdevScal .eq. 1) then
            call ESMF_StateGet(OBS_Pert_State, "Observation01", pertfield, &
                               rc=status)
            call LIS_verify(status, 'Error: StateGet Observation01')

            allocate (ssdev(LIS_rc%obs_ngrid(k)))
            ssdev = DUMMYsmlai_struc(n)%ssdev_inp

            if (DUMMYsmlai_struc(n)%ntimes .eq. 1) then
               jj = 1
            else
               jj = LIS_rc%mo
            endif
            do t = 1, LIS_rc%obs_ngrid(k)
               if (DUMMYsmlai_struc(n)%obs_sigma(t, jj) .gt. 0) then
                  ssdev(t) = ssdev(t)*DUMMYsmlai_struc(n)%model_sigma(t, jj)/ &
                             DUMMYsmlai_struc(n)%obs_sigma(t, jj)
...
end subroutine read_DUMMYsmlai

!BOP
! 
! !ROUTINE: read_DUMMYL2sm_data
! \label{read_DUMMYL2sm_data}
!
! !INTERFACE:
subroutine read_DUMMYL2sm_data(n, k,fname, smobs_inp, time)
! 
! !USES:   
...
!YK
  ...
  integer                        :: c,r,t
  logical*1                      :: sm_data_b(DUMMYsmlai_struc(n)%nc*DUMMYsmlai_struc(n)%nr)
  logical*1                      :: smobs_b_ip(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k))
  real                           :: sm_data(DUMMYsmlai_struc(n)%nc*DUMMYsmlai_struc(n)%nr)
  ...
  call LIS_verify(status, 'Error opening DUMMY L2 file ')

  call h5gopen_f(file_id,sm_gr_name,sm_gr_id, status)
  call LIS_verify(status, 'Error opening SM group in DUMMY L2 file')

  call h5dopen_f(sm_gr_id,sm_field_name,sm_field_id, status)
  call LIS_verify(status, 'Error opening SM field in DUMMY L2 file')

  call h5dopen_f(sm_gr_id,"EASE_row_index",row_id, status)
  call LIS_verify(status, 'Error opening row index field in DUMMY L2 file')

  call h5dopen_f(sm_gr_id,"EASE_column_index",col_id, status)
  call LIS_verify(status, 'Error opening column index field in DUMMY L2 file')

!YK
  call h5dopen_f(sm_gr_id, sm_qa_name,sm_qa_id, status)
  call LIS_verify(status, 'Error opening QA field in DUMMY L2 file')

!YK 
  call h5dopen_f(sm_gr_id, vwc_field_name,vwc_field_id, status)
  call LIS_verify(status, 'Error opening Veg water content field in DUMMY L2 file')

  call h5dget_space_f(sm_field_id, dspace_id, status)
  call LIS_verify(status, 'Error in h5dget_space_f: reaDUMMY L2Obs')

! Size of the arrays
! This routine returns -1 on failure, rank on success. 
  call h5sget_simple_extent_dims_f(dspace_id, dims, maxdims, status)
  if(status.eq.-1) then
     call LIS_verify(status, 'Error in h5sget_simple_extent_dims_f: readDUMMY L2Obs')
  endif
  ...
  call LIS_verify(status, 'Error extracting row index from DUMMY L2 file')

  call h5dread_f(col_id, H5T_NATIVE_INTEGER,ease_col,dims,status)
  call LIS_verify(status, 'Error extracting col index from DUMMY L2 file')

  call h5dread_f(sm_field_id, H5T_NATIVE_REAL,sm_field,dims,status)
  call LIS_verify(status, 'Error extracting SM field from DUMMY L2 file')
...
!YK get the vegetation water content
  ...
  call LIS_verify(status, 'Error extracting Veg water content (AM) field from DUMMY L2 file')
...
!        sm_data(ease_col(t) + &
!             (ease_row(t)-1)*DUMMYsmlai_struc(n)%nc) = sm_field(t)
!        if(sm_field(t).ne.-9999.0) then
!           sm_data_b(ease_col(t) + &
!                (ease_row(t)-1)*DUMMYsmlai_struc(n)%nc) = .true.
!        endif
!     endif
!  enddo
  ...
  do t=1,maxdims(1)
     if(ease_col(t).gt.0.and.ease_row(t).gt.0) then
        ...
        if(vwc_field(t).gt.5) then !YK Aply QC : if VWC > 5 kg/m2
           sm_data(ease_col(t) + &
                (ease_row(t)-1)*DUMMYsmlai_struc(n)%nc) = LIS_rc%udef
        else
           if(sm_data(ease_col(t) + &
                   (ease_row(t)-1)*DUMMYsmlai_struc(n)%nc).ne.-9999.0) then
...
!-------------------------------------------------------------------------- 
  call neighbor_interp(LIS_rc%obs_gridDesc(k,:), sm_data_b, sm_data, &
       smobs_b_ip, smobs_ip, &
       DUMMYsmlai_struc(n)%nc*DUMMYsmlai_struc(n)%nr, &
       LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k), &
       DUMMYsmlai_struc(n)%rlat, DUMMYsmlai_struc(n)%rlon,&
       DUMMYsmlai_struc(n)%n11, LIS_rc%udef, ios)
...
end subroutine read_DUMMYL2sm_data

!BOP
! 
! !ROUTINE: read_DUMMY_E_data
! \label{read_DUMMY_E_data}
!
! !INTERFACE:
subroutine read_DUMMY_E_data(n, k, pass, fname, smobs_ip)
! 
! !USES:   
...
!  \begin{description}
!  \item[n]            index of the nest
!  \item[fname]        name of the RTDUMMY AMSR-E file
...
! MN 
  ...
  dimsm      = (/DUMMYsmlai_struc(n)%nc, DUMMYsmlai_struc(n)%nr/)
  count_file = (/DUMMYsmlai_struc(n)%nc, DUMMYsmlai_struc(n)%nr/)
  count_mem  = (/DUMMYsmlai_struc(n)%nc, DUMMYsmlai_struc(n)%nr/)

  allocate(sm_field(DUMMYsmlai_struc(n)%nc, DUMMYsmlai_struc(n)%nr))
  allocate(sm_qa(DUMMYsmlai_struc(n)%nc, DUMMYsmlai_struc(n)%nr))
  allocate(vwc_field(DUMMYsmlai_struc(n)%nc, DUMMYsmlai_struc(n)%nr))
  allocate(dims(2))

  dims(1) = DUMMYsmlai_struc(n)%nc
  dims(2) = DUMMYsmlai_struc(n)%nr

  call h5open_f(status)
  call LIS_verify(status, 'Error opening HDF fortran interface')

  call h5fopen_f(trim(fname),H5F_ACC_RDONLY_F, file_id, status) 
  call LIS_verify(status, 'Error opening DUMMY file ')

  if(pass.eq.'D') then 
     call h5gopen_f(file_id,sm_gr_name_D,sm_gr_id_D, status)
     call LIS_verify(status, 'Error opening SM group in DUMMY file')

     call h5dopen_f(sm_gr_id_D,sm_field_name_D,sm_field_id_D, status)
     call LIS_verify(status, 'Error opening SM field in DUMMY file')

     call h5dget_space_f(sm_field_id_D, dataspace, status)
     call LIS_verify(status, 'Error in h5dget_space_f: readDUMMYObs')

     call h5sselect_hyperslab_f(dataspace, H5S_SELECT_SET_F, &
          start=offset_file, count=count_file, hdferr=status)
     call LIS_verify(status, 'Error setting hyperslab dataspace in readDUMMYObs')

     call h5screate_simple_f(memrank,dimsm, memspace, status)
     call LIS_verify(status, 'Error in h5create_simple_f; read_DUMMYsmlai')

     call h5sselect_hyperslab_f(memspace, H5S_SELECT_SET_F, &
          start=offset_mem, count=count_mem, hdferr=status)
     call LIS_verify(status, 'Error in h5sselect_hyperslab_f: read_DUMMYsmlai')

     call h5dread_f(sm_field_id_D, H5T_NATIVE_REAL,sm_field,dims,status, &
          memspace, dataspace)
     call LIS_verify(status, 'Error extracting SM field from DUMMYfile')

     call h5dclose_f(sm_field_id_D,status)
     call LIS_verify(status,'Error in H5DCLOSE call')

     call h5dopen_f(sm_gr_id_D,sm_qa_name_D,sm_qa_id_D, status)
     call LIS_verify(status, 'Error opening SM QA field in DUMMY file')

     call h5dread_f(sm_qa_id_D, H5T_NATIVE_INTEGER,sm_qa,dims,status, &
          memspace, dataspace)
     call LIS_verify(status, 'Error extracting SM QA field from DUMMYfile')
...
! MN get the vegetation water contnent 
     ...
     call LIS_verify(status, 'Error opening Veg water content field in DUMMY file')

     call h5dread_f(vwc_field_id_D, H5T_NATIVE_REAL,vwc_field,dims,status, &
          memspace, dataspace)
     call LIS_verify(status, 'Error extracting Veg water content (AM) field from DUMMYfile')
...
! indicates the recommended quality (0-means retrieval has recommended quality).
  ...
  do r=1,DUMMYsmlai_struc(n)%nr
     do c=1,DUMMYsmlai_struc(n)%nc        
...
end subroutine read_DUMMY_E_data

! MN: the data structure in both 36 km and 9 km products is the same therefore  
!         read_DUMMY_E_data is similar to read_DUMMY_data

!BOP
! 
! !ROUTINE: read_DUMMY_data
! \label{read_DUMMY_data}
!
! !INTERFACE:
subroutine read_DUMMY_data(n, k, pass, fname, smobs_ip)
! 
! !USES:   
...
end subroutine read_DUMMY_data

#if 0 
!BOP
! !ROUTINE: create_DUMMYsmlai_filename
! \label{create_DUMMYsmlai_filename}
! 
! !INTERFACE: 
subroutine create_DUMMYsmlai_filename(ndir, designation, yr, mo,da, filename)
! !USES:   
...
! 
! !DESCRIPTION: 
!  This subroutine creates the DUMMY filename (from NSIDC)
!  based on the time and date 
! 
!  The arguments are: 
!  \begin{description}
!  \item[ndir] name of the DUMMY soil moisture data directory
!  \item[yr]  current year
!  \item[mo]  current month
!  \item[da]  current day
!  \item[filename] Generated DUMMY filename
! \end{description}
!EOP
  ...
  if(designation.eq."SPL3DUMMY") then 
     filename = trim(ndir)//'/'//trim(fyr)//'.'//trim(fmo)//'.'//&
          trim(fda)//'/DUMMY_L3_SM_AP_'&
          //trim(fyr)//trim(fmo)//trim(fda)//&
          '_R12170_001.h5'
! MN:   
! The DUMMY file names contain components that change in a way that
! is difficult to programatically generate. So after downloading
! a DUMMY data file, a symbolic link was created to it which make it 
! easier to generate file name.
!   For example:
!   DUMMY_L3_SM_P_20170902.h5 -> DUMMY_L3_SM_P_20170902_R15152_001.h5
  elseif(designation.eq."SPL3SMP") then 
     filename = trim(ndir)//'/'//trim(fyr)//'.'//trim(fmo)//'.'//&
          trim(fda)//'/DUMMY_L3_SM_P_'&
          //trim(fyr)//trim(fmo)//trim(fda)//&
         '.h5'
! For example:
! DUMMY_L3_SM_P_E_20180811.h5 -> DUMMY_L3_SM_P_E_20180811_R16010_001.h5
! MN for sake of convenience instead of real name the symbolic link was used
  elseif(designation.eq."SPL3SMP_E") then 
     filename = trim(ndir)//'/'//trim(fyr)//'.'//trim(fmo)//'.'//&
          trim(fda)//'/DUMMY_L3_SM_P_E_'&
          //trim(fyr)//trim(fmo)//trim(fda)//&
          '.h5'

  endif

end subroutine create_DUMMYsmlai_filename

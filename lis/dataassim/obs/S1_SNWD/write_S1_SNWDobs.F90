...
!BOP
! 
! !ROUTINE: write_S1_SNWDobs
! \label{write_S1_SNWDobs}
! 
! !REVISION HISTORY: 
! 30 Aug 2019: Hans Lievens; Initial Specification
! 
! !INTERFACE: 
subroutine write_S1_SNWDobs(n, k, OBS_State)
! !USES: 
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
!  real                     :: testdata(LIS_rc%obs_lnc(k), LIS_rc%obs_lnr(k))
  ...
  if(data_update) then 
     ...
     if(LIS_masterproc) then 
        ...
        call S1_SNWD_obsname(n,k,obsname)        
...
end subroutine write_S1_SNWDobs

!BOP
! !ROUTINE: S1_SNWD_obsname
! \label{S1_SNWD_obsname}
! 
! !INTERFACE: 
subroutine S1_SNWD_obsname(n,k,obsname)
! !USES: 
...
end subroutine S1_SNWD_obsname

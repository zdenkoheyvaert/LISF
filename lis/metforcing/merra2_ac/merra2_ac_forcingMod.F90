...
!-----------------------------------------------------------------------------
  public :: init_merra2_ac      !defines the native resolution of
                             !the input data
!-----------------------------------------------------------------------------
! !PUBLIC TYPES:
!-----------------------------------------------------------------------------
  public :: merra2_ac_struc

!EOP
  type, public ::  merra2_ac_type_dec
     real         :: ts
     integer      :: ncold, nrold
  ...
  end type merra2_ac_type_dec

  type(merra2_ac_type_dec), allocatable :: merra2_ac_struc(:)

contains

!BOP
!
! !ROUTINE: init_merra2_ac
...
!  \begin{description}
!   \item[readcrd\_merra2_ac](\ref{readcrd_merra2_ac}) \newline
...
!EOP
    ...
    allocate(merra2_ac_struc(LIS_rc%nnest))

    do n=1,LIS_rc%nnest
       merra2_ac_struc(n)%ncold = 576
       merra2_ac_struc(n)%nrold = 361
    enddo

    call readcrd_merra2_ac()
    LIS_rc%met_nf(findex) = 18
    ...
    do n=1,LIS_rc%nnest
       gridDesci(n,1) = 0
       gridDesci(n,2) = merra2_ac_struc(n)%ncold
       gridDesci(n,3) = merra2_ac_struc(n)%nrold
       ...
       merra2_ac_struc(n)%mi = merra2_ac_struc(n)%ncold*merra2_ac_struc(n)%nrold

       ! Setting up weights for Interpolation
       if(trim(LIS_rc%met_interp(findex)).eq."bilinear") then
          allocate(merra2_ac_struc(n)%n111(LIS_rc%lnc(n)*LIS_rc%lnr(n)))
          allocate(merra2_ac_struc(n)%n121(LIS_rc%lnc(n)*LIS_rc%lnr(n)))
          allocate(merra2_ac_struc(n)%n211(LIS_rc%lnc(n)*LIS_rc%lnr(n)))
          allocate(merra2_ac_struc(n)%n221(LIS_rc%lnc(n)*LIS_rc%lnr(n)))
          allocate(merra2_ac_struc(n)%w111(LIS_rc%lnc(n)*LIS_rc%lnr(n)))
          allocate(merra2_ac_struc(n)%w121(LIS_rc%lnc(n)*LIS_rc%lnr(n)))
          allocate(merra2_ac_struc(n)%w211(LIS_rc%lnc(n)*LIS_rc%lnr(n)))
          allocate(merra2_ac_struc(n)%w221(LIS_rc%lnc(n)*LIS_rc%lnr(n)))
       ...
       elseif(trim(LIS_rc%met_interp(findex)).eq."budget-bilinear") then
          ...
          allocate(merra2_ac_struc(n)%n112(LIS_rc%lnc(n)*LIS_rc%lnr(n),25))
          allocate(merra2_ac_struc(n)%n122(LIS_rc%lnc(n)*LIS_rc%lnr(n),25))
          allocate(merra2_ac_struc(n)%n212(LIS_rc%lnc(n)*LIS_rc%lnr(n),25))
          allocate(merra2_ac_struc(n)%n222(LIS_rc%lnc(n)*LIS_rc%lnr(n),25))
          allocate(merra2_ac_struc(n)%w112(LIS_rc%lnc(n)*LIS_rc%lnr(n),25))
          allocate(merra2_ac_struc(n)%w122(LIS_rc%lnc(n)*LIS_rc%lnr(n),25))
          allocate(merra2_ac_struc(n)%w212(LIS_rc%lnc(n)*LIS_rc%lnr(n),25))
          allocate(merra2_ac_struc(n)%w222(LIS_rc%lnc(n)*LIS_rc%lnr(n),25))
       ...
       elseif(trim(LIS_rc%met_interp(findex)).eq."neighbor") then
          allocate(merra2_ac_struc(n)%n113(LIS_rc%lnc(n)*LIS_rc%lnr(n)))
          call neighbor_interp_input(n, gridDesci(n,:),&
               merra2_ac_struc(n)%n113)
       ...
       if(LIS_rc%forecastMode.eq.1) then 
          ...
          allocate(merra2_ac_struc(n)%merraforc1(&
               LIS_forecast_struc(1)%niterations,&
               merra2_ac_struc(n)%nvars, 24, &
               LIS_rc%lnc(n)*LIS_rc%lnr(n)))
          allocate(merra2_ac_struc(n)%merraforc2(&
               LIS_forecast_struc(1)%niterations,&
               merra2_ac_struc(n)%nvars, 24, &
               LIS_rc%lnc(n)*LIS_rc%lnr(n)))

          merra2_ac_struc(n)%st_iterid = LIS_forecast_struc(1)%st_iterId
          merra2_ac_struc(n)%en_iterId = LIS_forecast_struc(1)%niterations
          merra2_ac_struc(n)%nIter = LIS_forecast_struc(1)%niterations
          
          allocate(merra2_ac_struc(n)%metdata1(LIS_forecast_struc(1)%niterations,&
               LIS_rc%met_nf(findex),&
               LIS_rc%ngrid(n)))
          allocate(merra2_ac_struc(n)%metdata2(LIS_forecast_struc(1)%niterations,&
               LIS_rc%met_nf(findex),&
               LIS_rc%ngrid(n)))
          
       ! Regular retrospective or non-forecast mode:
       else
          allocate(merra2_ac_struc(n)%merraforc1(1,&
               merra2_ac_struc(n)%nvars, 24, &
               LIS_rc%lnc(n)*LIS_rc%lnr(n)))
          allocate(merra2_ac_struc(n)%merraforc2(1,&
               merra2_ac_struc(n)%nvars, 24, &
               LIS_rc%lnc(n)*LIS_rc%lnr(n)))
          merra2_ac_struc(n)%st_iterid = 1
          merra2_ac_struc(n)%en_iterId = 1
          merra2_ac_struc(n)%nIter = 1
          
          allocate(merra2_ac_struc(n)%metdata1(1,LIS_rc%met_nf(findex),&
               LIS_rc%ngrid(n)))
          allocate(merra2_ac_struc(n)%metdata2(1,LIS_rc%met_nf(findex),&
               LIS_rc%ngrid(n)))
          
       endif

       merra2_ac_struc(n)%metdata1 = 0
       merra2_ac_struc(n)%metdata2 = 0

       merra2_ac_struc(n)%merraforc1 = LIS_rc%udef
       merra2_ac_struc(n)%merraforc2 = LIS_rc%udef

       if ( LIS_rc%met_ecor(findex) == "lapse-rate" .or. &
            LIS_rc%met_ecor(findex) == "lapse-rate and slope-aspect" ) then
          call read_merra2_ac_elev(n,findex)
       endif
  ...
  end subroutine init_merra2_ac
end module merra2_ac_forcingMod

...
! !ARGUMENTS: 
  ...
  character(len=*), intent(in) :: ac70_daily_name
...
! Read single layer file (*) fields:
  ...
  if(file_exists) then 
     write(LIS_logunit,*)'[INFO] Reading MERRA-2 ac70 file ... ',trim(ac70_daily_name)
     call LIS_verify(nf90_open(path=trim(ac70_daily_name), mode=NF90_NOWRITE, &
          ncid=ftn), 'nf90_open failed for ac70 daily file in read_merra2_ac')

...
!BOP
! !ROUTINE: noahmp401_qcsoilmLAIveg
! \label{noahmp401_qcsoilmLAIveg}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmp401_qcsoilmLAI(n, LSM_State)
...
!  real, pointer          :: soilm4(:)
  ...
  call LIS_verify(status,&
       "ESMF_StateGet for Soil Moisture Layer 1 failed in noahmp401_qcsoilmLAI")
 
  call ESMF_FieldGet(sm1Field,localDE=0,farrayPtr=soilm1,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet for Soil Moisture Layer 1 failed in noahmp401_qcsoilmLAI")

  call ESMF_AttributeGet(sm1Field,"Max Value",smmax1,rc=status)
  call LIS_verify(status,&
       "ESMF_AttributeGet: Max Value failed in noahmp401_qcsoilmLAI")

  call ESMF_AttributeGet(sm1Field,"Min Value",smmin1,rc=status)
  call LIS_verify(status,&
       "ESMF_AttributeGet: Min Value failed in noahmp401_qcsoilmLAI")

  call ESMF_StateGet(LSM_State,"LAI",laiField,rc=status)
  call LIS_verify(status,&
           "ESMF_StateGet for LAI failed in noahmp401_qcsoilmLAI")
  call ESMF_FieldGet(laiField,localDE=0,farrayPtr=lai,rc=status)
  call LIS_verify(status,&
           "ESMF_FieldGet for LAI failed in noahmp401_qcsoilmLAI")

  call ESMF_AttributeGet(laiField,"Max Value",laimax,rc=status)
  call LIS_verify(status,&
           "ESMF_AttributeGet for LAI Max Value failed in noahmp401_qcsoilmLAI")
  call ESMF_AttributeGet(laiField,"Min Value",laimin,rc=status)
  call LIS_verify(status,&
           "ESMF_AttributeGet for LAI Min Value failed in noahmp401_qcsoilmLAI")
...
! update.
  ...
  write(113,fmt='(i4.4,i2.2,i2.2,i2.2,i2.2,i2.2,21F8.3)') &
       ...
       state_tmp
#endif
end subroutine noahmp401_qcsoilmLAI

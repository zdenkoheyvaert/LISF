...
!BOP
! !ROUTINE: noahmp401_setsoilmLAI
!  \label{noahmp401_setsoilmLAI}
!
! !REVISION HISTORY:
...
! !INTERFACE:
subroutine noahmp401_setsoilmLAI(n, LSM_State)
! !USES:
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
! mn
  ...
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 1 failed in noahmp401_setsoilmLAI")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 2",sm2Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 2 failed in noahmp401_setsoilmLAI")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 3",sm3Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 3 failed in noahmp401_setsoilmLAI")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 4",sm4Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 4 failed in noahmp401_setsoilmLAI")
  call ESMF_StateGet(LSM_State,"LAI",laiField,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: LAI failed in noahmp401_setsoilmLAI")

  call ESMF_FieldGet(sm1Field,localDE=0,farrayPtr=soilm1,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 1 failed in noahmp401_setsoilmLAI")
  call ESMF_FieldGet(sm2Field,localDE=0,farrayPtr=soilm2,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 2 failed in noahmp401_setsoilmLAI")
  call ESMF_FieldGet(sm3Field,localDE=0,farrayPtr=soilm3,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 3 failed in noahmp401_setsoilmLAI")
  call ESMF_FieldGet(sm4Field,localDE=0,farrayPtr=soilm4,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 4 failed in noahmp401_setsoilmLAI")
  call ESMF_FieldGet(laiField,localDE=0,farrayPtr=lai,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: LAI failed in noahmp401_setsoilmLAI")
 ...
 !       can get module variables MAXSMC and WLTSMC from the module_sf_noahmplsm_401       
...
!     XLAI    = max(LFMASS*LAPM,laimin)
     ...
     if(NOAHMP401_struc(n)%noahmp401(t)%param%SLA.ne.0) then 
        ...
        lfmass = lai(t)/(NOAHMP401_struc(n)%noahmp401(t)%param%SLA/1000.0)
        NOAHMP401_struc(n)%noahmp401(t)%lfmass = lfmass
     endif
  enddo

end subroutine noahmp401_setsoilmLAI

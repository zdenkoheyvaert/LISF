...
!BOP
! !ROUTINE: noahmp401_getSig0VVVHpred
! \label{noahmp401_getSig0VVVHpred}
!
! !REVISION HISTORY:
...
subroutine noahmp401_getSig0VVVHpred(n,k,obs_pred)
...
!LIS_histDAtaMod
  call LIS_verify(status, &
       "Error in StateGet in noahmp401_getSig0Pred for WCM_Sig0VV")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=s0VV, rc=status)!variable
!s0VV previously defined
  call LIS_verify(status, &
       'Error in FieldGet in noahmp401_getSig0Pred for WCM_Sig0VV')

  call ESMF_StateGet(LIS_forwardState(n), "WCM_Sig0VH", varField, rc=status)
  call LIS_verify(status, &
       "Error in StateGet in noahmp401_getSig0Pred for WCM_Sig0VH")

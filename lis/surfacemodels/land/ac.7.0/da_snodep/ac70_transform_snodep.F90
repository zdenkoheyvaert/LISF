...
!BOP
! !ROUTINE: ac70_transform_snodep
! \label{ac70_transform_snodep}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine ac70_transform_snodep(n,OBS_State)
...
!EOP
  ...
  call LIS_verify(status, 'attributeget error in ac70_transform_snodep')
  call ESMF_StateGet(OBS_State,"Observation01",obs_snodep_field,&
       rc=status)
  call LIS_verify(status,'stateget error in ac70_transform_snodep')
  call ESMF_FieldGet(obs_snodep_field,localDE=0,farrayPtr=snodepobs,rc=status)
  call LIS_verify(status,'fieldget error in ac70_transform_snodep')
...
end subroutine ac70_transform_snodep

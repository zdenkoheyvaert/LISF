...
!BOP
! !ROUTINE: ac70_snodep_update
! \label{ac70_snodep_update}
!
! !REVISION HISTORY:
!  13 Aug 2017: Sujay Kumar; Initial specification
!
! !INTERFACE:
subroutine ac70_snodep_update(n, t, dsneqv, dsnowh)
...
! local
  ...
  isnow = ac70_struc(n)%ac70(t)%isnow
  nsoil = ac70_struc(n)%nsoil
  nsnow = ac70_struc(n)%nsnow
...
!  psisat = satpsi (soiltyp)
  ...
  sneqv = ac70_struc(n)%ac70(t)%sneqv
  snowh = ac70_struc(n)%ac70(t)%snowh

  zsnso(-nsnow+1:nsoil) = ac70_struc(n)%ac70(t)%zss(1:nsnow+nsoil) 
...
! snow/soil layer thickness (m)
  ...
  do i = 2, nsoil
     zsoil(i) = zsoil(i-1) - AC70_struc(n)%sldpth(i)
  enddo


  ! state variables 
  snice(-nsnow+1:0) = &
       AC70_struc(n)%ac70(t)%snowice(1:nsnow)
  snliq(-nsnow+1:0) = &
       AC70_struc(n)%ac70(t)%snowliq(1:nsnow) 
  stc(-nsnow+1:nsoil) = &
       AC70_struc(n)%ac70(t)%sstc(1:nsnow+&
       nsoil) 
! from snowfall routine
  ...
  IF(ISNOW == 0 .AND. SNOWH >= 0.025.and.&
     ...
     DZSNSO(0)= SNOWH
     SNOWH    = 0.
     STC(0)   = MIN(273.16, AC70_struc(n)%ac70(t)%sfctmp)   ! temporary setup
     SNICE(0) = SNEQV
     SNLIQ(0) = 0.
  END IF
...
! how do you determine the thickness of a layer?
  ...
  sice(:) = max(0.0, AC70_struc(n)%ac70(t)%smc(:)&
       - AC70_struc(n)%ac70(t)%sh2o(:))   
...
!imelt
  ...
  do j = 1, nsoil               ! soil
     mliq(j) =  AC70_struc(n)%ac70(t)%sh2o(j) * dzsnso(j) * 1000.
     mice(j) = (AC70_struc(n)%ac70(t)%smc(j) - &
          AC70_struc(n)%ac70(t)%sh2o(j))  * dzsnso(j) * 1000.
  end do
...
!     end if
  ...
  call  snowh2o (nsnow  ,&
       nsoil  ,&
       ac70_struc(n)%dt     ,qsnfro ,qsnsub , & !in 
       qrain  ,iloc   ,jloc   ,                 & !in
       isnow  ,&
       dzsnso ,snowh  ,sneqv  ,snice  , & !inout
       snliq  ,ac70_struc(n)%ac70(t)%sh2o   ,&
       sice   ,ac70_struc(n)%ac70(t)%stc    ,         & !inout
...
! Reset ZSNSO and layer thinkness DZSNSO
  ...
  ac70_struc(n)%ac70(t)%sneqv = sneqv
  ac70_struc(n)%ac70(t)%snowh = snowh 

  AC70_struc(n)%ac70(t)%zss(1:nsnow+&
       nsoil) = & 
       zsnso(-nsnow+1:nsoil)
...
end subroutine ac70_snodep_update

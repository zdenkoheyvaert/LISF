...
!BOP
! !ROUTINE: ac70_map_snodep
! \label{ac70_map_snodep}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine ac70_map_snodep(n,k,OBS_State,LSM_Incr_State)
! !USES:
  use ESMF
...
!EOP
  ...
  real, allocatable            :: ac70_swe(:)
  real, allocatable            :: ac70_snod(:)
  real, allocatable            :: snod(:)

  allocate(ac70_swe(LIS_rc%npatch(n,LIS_rc%lsm_index)))
  allocate(ac70_snod(LIS_rc%npatch(n,LIS_rc%lsm_index)))
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ac70_swe(t)  = ac70_struc(n)%ac70(t)%sneqv
     ac70_snod(t) = ac70_struc(n)%ac70(t)%snowh
  enddo
...
! working with an model grid finer than the observation grid
     ...
     if(snodepobs(st_id).ge.0) then 
        if(ac70_snod(t).gt.1e-6) then 
           tmpsneqv = ac70_swe(t)/ac70_snod(t)
        else
           tmpsneqv = 0.0
        endif
...
! Based on SNODEP, we manually update SWE
        ...
        sweincr(t) = tmpsneqv*snod(t) - ac70_swe(t)
        snodincr(t) = snod(t) - ac70_snod(t)
     else
        sweincr(t) = 0 
        snodincr(t) = 0 
     endif
...
!  stop
  ...
  deallocate(ac70_swe)
  deallocate(ac70_snod)
  deallocate(snod)

end subroutine ac70_map_snodep

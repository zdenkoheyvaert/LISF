...
!BOP
! !ROUTINE: ac70_getsnodeppred
! \label{ac70_getsnodeppred}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine ac70_getsnodeppred(n, k, obs_pred)
...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     if (SNODEP_obs_obj(n)%mesh .eq. 8) then
        snwd(t) = ac70_struc(n)%ac70(t)%snowh*39.37 !convert from meter to inch
     elseif (SNODEP_obs_obj(n)%mesh .eq. 16 .or. SNODEP_obs_obj(n)%mesh .eq. 25) then
        snwd(t) = ac70_struc(n)%ac70(t)%snowh
     endif
  enddo
...
end subroutine ac70_getsnodeppred

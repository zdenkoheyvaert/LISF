...
!
! !DESCRIPTION:
!  This program writes restart files for Ac70.
...
! \item[Ac70\_dump\_restart](\ref{Ac70_dump_restart}) \newline
!   writes the Ac70 variables into the restart file
! \end{description}
!EOP
    ...
    ! set restart alarm
    alarmCheck = LIS_isAlarmRinging(LIS_rc, "Ac70 restart alarm")
    ...
    if(alarmCheck .or. (LIS_rc%endtime ==1)) then
        If (LIS_masterproc) Then
            ...
            call LIS_create_restart_filename(n, filen, "SURFACEMODEL", "AC70",&
                                             wformat=wformat)
            ...
            elseif(wformat .eq. "netcdf") then
                ...
                call LIS_verify(status,"Error in nf90_open in Ac70_writerst")
#endif
        ...
        call Ac70_dump_restart(n, ftn, wformat)
        ...
        if (LIS_masterproc) then
            ...
            elseif(wformat .eq. "netcdf") then
                ...
                call LIS_verify(status, "Error in nf90_close in Ac70_writerst")
#endif
            endif
            write(LIS_logunit, *) "Ac70 archive restart written: ", filen
        endif
    endif
end subroutine Ac70_writerst

!BOP
!
! !ROUTINE: Ac70_dump_restart
! \label{Ac70_dump_restart}
!
! !REVISION HISTORY:
!  This subroutine is generated with the Model Implementation Toolkit developed
!  by Shugong Wang for the NASA Land Information System Version 7. The initial 
...
!  9/4/14: Shugong Wang, initial implementation for LIS 7 and Ac70
! !INTERFACE:
subroutine Ac70_dump_restart(n, ftn, wformat)
...
!
!
!  The following is the list of variables written in the Ac70
!  restart file:
!  \begin{verbatim}
...
!EOP 
    ...
    call LIS_writeGlobalHeader_restart(ftn, n, LIS_rc%lsm_index, &
         "AC70", &
         dim1 = AC70_struc(n)%nsoil+AC70_struc(n)%nsnow,   &
         dim2 = AC70_struc(n)%nsoil, &
         dim3 = AC70_struc(n)%nsnow, & 
         dimID = dimID, &
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, sstc_ID, "SSTC", &
         ...
         "K", vlevels=AC70_struc(n)%nsoil+AC70_struc(n)%nsnow, &
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, sh2o_ID, "SH2O", &
         ...
         "m^3 m-3", vlevels=AC70_struc(n)%nsoil, valid_min=-99999.0, valid_max=99999.0, &
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, zss_ID, "ZSS", &
         ...
         "m", vlevels=AC70_struc(n)%nsoil+AC70_struc(n)%nsnow , valid_min=-99999.0, valid_max=99999.0, &
         var_flag = "dim1") ! TODO: replace "xxxx" with correct "dimx"
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, snowice_ID, "SNOWICE", &
         "snow-layer ice", &
         "mm", vlevels=AC70_struc(n)%nsnow , valid_min=-99999.0, valid_max=99999.0, &
    ...
    call LIS_closeHeader_restart(ftn, n, LIS_rc%lsm_index, dimID, AC70_struc(n)%rstInterval)
    
    ! write state variables into restart file
    ! snow albedo at last time step
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%albold, &
         varid=albold_ID, dim=1, wformat=wformat)
    
    ! snow mass at the last time step
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%sneqvo, &
         varid=sneqvo_ID, dim=1, wformat=wformat)

    ! snow/soil temperature
    do l=1, AC70_struc(n)%nsoil+AC70_struc(n)%nsnow   ! TODO: check loop
       tmptilen = 0
       do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
          tmptilen(t) = AC70_struc(n)%ac70(t)%sstc(l)
       enddo
    ...
    do l=1, AC70_struc(n)%nsoil   ! TODO: check loop
       tmptilen = 0
       do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
          tmptilen(t) = AC70_struc(n)%ac70(t)%sh2o(l)
       enddo
       call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, AC70_struc(n)%nsoil   ! TODO: check loop
       tmptilen = 0
       do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
          tmptilen(t) = AC70_struc(n)%ac70(t)%smc(l)
       enddo
       call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
            varid=smc_ID, dim=l, wformat=wformat)
    enddo
    ! canopy air temperature
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%tah, &
         varid=tah_ID, dim=1, wformat=wformat)

    ! canopy air vapor pressure
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%eah, &
         varid=eah_ID, dim=1, wformat=wformat)

    ! wetted or snowed fraction of canopy
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%fwet, &
         varid=fwet_ID, dim=1, wformat=wformat)

    ! intercepted liquid water
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%canliq, &
         varid=canliq_ID, dim=1, wformat=wformat)

    ! intercepted ice mass
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%canice, &
         varid=canice_ID, dim=1, wformat=wformat)

    ! vegetation temperature
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%tv, &
         varid=tv_ID, dim=1, wformat=wformat)

    ! ground temperature (skin temperature)
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%tg, &
         varid=tg_ID, dim=1, wformat=wformat)

    ! snowfall on the ground
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%qsnow, &
         varid=qsnow_ID, dim=1, wformat=wformat)

    ! actual number of snow layers
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%isnow, &
    ...
    do l=1, AC70_struc(n)%nsoil+AC70_struc(n)%nsnow   ! TODO: check loop
       tmptilen = 0
       do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
          tmptilen(t) = AC70_struc(n)%ac70(t)%zss(l)
       enddo
       call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! snow height
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%snowh, &
         varid=snowh_ID, dim=1, wformat=wformat)

    ! snow water equivalent
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%sneqv, &
    ...
    do l=1, AC70_struc(n)%nsnow   ! TODO: check loop
       tmptilen = 0
       do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
          tmptilen(t) = AC70_struc(n)%ac70(t)%snowice(l)
       enddo
       call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, AC70_struc(n)%nsnow   ! TODO: check loop
       tmptilen = 0
       do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
          tmptilen(t) = AC70_struc(n)%ac70(t)%snowliq(l)
       enddo
       call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
            varid=snowliq_ID, dim=l, wformat=wformat)
    enddo
    ! depth to water table
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%zwt, &
         varid=zwt_ID, dim=1, wformat=wformat)

    ! water storage in aquifer
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%wa, &
         varid=wa_ID, dim=1, wformat=wformat)

    ! water in aquifer and saturated soil
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%wt, &
         varid=wt_ID, dim=1, wformat=wformat)

    ! lake water storage
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%wslake, &
         varid=wslake_ID, dim=1, wformat=wformat)

    ! leaf mass
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%lfmass, &
         varid=lfmass_ID, dim=1, wformat=wformat)

    ! mass of fine roots
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%rtmass, &
         varid=rtmass_ID, dim=1, wformat=wformat)

    ! stem mass
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%stmass, &
         varid=stmass_ID, dim=1, wformat=wformat)

    ! mass of wood including woody roots
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%wood, &
         varid=wood_ID, dim=1, wformat=wformat)

    ! stable carbon in deep soil
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%stblcp, &
         varid=stblcp_ID, dim=1, wformat=wformat)

    ! short-lived carbon in shallow soil
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%fastcp, &
         varid=fastcp_ID, dim=1, wformat=wformat)

    ! leaf area index
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%lai, &
         varid=lai_ID, dim=1, wformat=wformat)

    ! stem area index
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%sai, &
         varid=sai_ID, dim=1, wformat=wformat)

    ! momentum drag coefficient
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%cm, &
         varid=cm_ID, dim=1, wformat=wformat)

    ! sensible heat exchange coefficient
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%ch, &
         varid=ch_ID, dim=1, wformat=wformat)

    ! snow aging term
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%tauss, &
         varid=tauss_ID, dim=1, wformat=wformat)

    ! soil water content between bottom of the soil and water table
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%smcwtd, &
         varid=smcwtd_ID, dim=1, wformat=wformat)

    ! recharge to or from the water table when deep
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%deeprech, &
         varid=deeprech_ID, dim=1, wformat=wformat)

    ! recharge to or from the water table when shallow
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%rech, &
         varid=rech_ID, dim=1, wformat=wformat)

    ! reference height for air temperature and humidity 
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, AC70_struc(n)%ac70%zlvl, &
         varid=zlvl_ID, dim=1, wformat=wformat)

end subroutine Ac70_dump_restart

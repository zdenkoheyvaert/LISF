...
#define LIS_Ac_TEST 0
! !INTERFACE
subroutine ac_driver_70(iloc, jloc, &
     landuse_tbl_name, soil_tbl_name,  gen_tbl_name,             & 
                            ac_tbl_name, landuse_scheme_name, soil_scheme_name,     &
  ...
  use ac_routines_70, only: ac_sflx_70, redprm 
  implicit none
  ...
  character(len=256), intent(in) :: ac_tbl_name       ! Ac parameter table
  ...
  write(*,*) " Ac parameter table: ",              ac_tbl_name     
...
!  jloc = 1 
  ...
  ! cosz, yearlen, and julian are calculated in subroutine calc_declin_70 
  ! be careful here!!!, LIS uses GMT. the date for calc_declin_70 should be local time. Longitude is need to cnvert GMT 
  ! into local time!!!. To be implemented after bondville test 
  write(nowdate,'(I4.4,4I2.2)') year, month, day, hour, minute
  call calc_declin_70(nowdate(1:4)//"-"//nowdate(5:6)//"-"//nowdate(7:8)//"_"//nowdate(9:10)//":"//nowdate(11:12)//":00", &
      latitude, longitude, cosz, yearlen, julian)

  ! SY: Begin for enabling OPTUE
  ! SY: Begin lines following those in REDPRM and Ac70_setup
  ...
  ! SY: End lines following those in REDPRM and Ac70_setup
  ! SY: Begin lines following those in read_mp_veg_parameters and Ac70_setup
  ...
  MRP(vegetype) = p_MRP
  ! SY: End lines following those in read_mp_veg_parameters and Ac70_setup
  ...
  if ( dveg == 1 ) then
    ...
    shdfac = month_d_70(shdfac_monthly, nowdate)
  else
  ...
  foln    = 1.0              ! foliage nitrogen, (fraction) a kind of forcing from WRF? 
                             ! in simple driver, it is set to 1.0. In Ac code, it 
  ...
  deallocate(snliq)
  deallocate(stc)
end subroutine ac_driver_70

real function month_d_70(a12, nowdate) result (nowval)
  !
...
end function month_d_70 

SUBROUTINE calc_declin_70 ( nowdate, latitude, longitude, cosz, yearlen, julian)
  use kwm_date_utilities_70 
!---------------------------------------------------------------------
  IMPLICIT NONE
!---------------------------------------------------------------------
...
END SUBROUTINE calc_declin_70 

! Subroutine SNOW_INIT grabbed from NOAH-MP-WRF
SUBROUTINE SNOW_INIT_70 ( jts, jtf, its, itf, ims, ime, jms, jme, NSNOW, NSOIL, ZSOIL,  &

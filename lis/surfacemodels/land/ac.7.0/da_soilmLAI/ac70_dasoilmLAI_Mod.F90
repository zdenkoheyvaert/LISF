...
!BOP
! 
! !ROUTINE: ac70_dasoilmLAI_init
! \label{ac70_dasoilmLAI_init}
! 
! !INTERFACE:
  subroutine ac70_dasoilmLAI_init(k)
! !USES:
! !DESCRIPTION:        
!
!EOP
...
!TBD: SVK
    ...
    if(LIS_rc%dascaloption(k).eq."Linear scaling") then 
       call ESMF_ConfigFindLabel(LIS_config,"ac70 soil moisture CDF file:",&
            rc=status)
       do n=1,LIS_rc%nnest
          call ESMF_ConfigGetAttribute(LIS_config,modelcdffile(n),rc=status)
          call LIS_verify(status, 'ac70 soil moisture CDF file: not defined')
       enddo
...
!Hardcoded for now.
  ...
  end subroutine ac70_dasoilmLAI_init
end module ac70_dasoilmLAI_Mod

...
!BOP
! !ROUTINE: ac70_getrunoffs_mm
!  \label{ac70_getrunoffs_mm}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine ac70_getrunoffs_mm(n)
! !USES:
  use ESMF
...
!gather the model tiles before assigning to the global data structure. 
  ...
  if(evapflag.ne.0)then
    ...
    if(LIS_masterproc) then 
      ...
      call LIS_verify(status, "ac70_getrunoffs_mm: ESMF_StateGet failed for Total Evapotranspiration")
        
      call ESMF_FieldGet(evapotranspiration_Field,localDE=0,farrayPtr=evapotranspiration,rc=status)
      call LIS_verify(status, "ac70_getrunoffs_mm: ESMF_FieldGet failed for Total Evapotranspiration")
...
end subroutine ac70_getrunoffs_mm

...
!BOP
! !ROUTINE: ac70_getrunoffs
!  \label{ac70_getrunoffs}
!
! !REVISION HISTORY:
...
!if option is not defined, then assume that no evap calculations will be done
  ...
  do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)  !units?
     runoff1(t) = AC70_struc(n)%ac70(t)%runsrf/LIS_CONST_RHOFW
     runoff2(t) = AC70_struc(n)%ac70(t)%runsub/LIS_CONST_RHOFW
  enddo
...
!gather the model tiles before assigning to the global data structure. 
  ...
  if(evapflag.ne.0)then
    ...
    if(LIS_masterproc) then 
      ...
      call LIS_verify(status, "ac70_getrunoffs: ESMF_StateGet failed for Total Evapotranspiration")
        
      call ESMF_FieldGet(evapotranspiration_Field,localDE=0,farrayPtr=evapotranspiration,rc=status)
      call LIS_verify(status, "ac70_getrunoffs: ESMF_FieldGet failed for Total Evapotranspiration")

    endif

    do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)  
       evapotranspiration1(t)  =  AC70_struc(n)%ac70(t)%ecan + AC70_struc(n)%ac70(t)%etran + AC70_struc(n)%ac70(t)%edir 
    enddo
...
end subroutine ac70_getrunoffs

...
!BOP
! !ROUTINE: ac70_getsws_hymap2
!  \label{ac70_getsws_hymap2}
!
! !REVISION HISTORY:
! 12 Sep 2019: Augusto Getirana; implementation of two-way coupling
!
! !INTERFACE:
subroutine ac70_getsws_hymap2(n)
! !USES:
  use ESMF
...
!
! !DESCRIPTION:
!   This routine defines the surface water storage variables in Ac
...
!EOP
  ...
  if(enable2waycpl==1) then 
     ...
     where(rivstotmp/=LIS_rc%udef) &
          AC70_struc(n)%ac70(:)%rivsto=rivstotmp/AC70_struc(n)%dt
     ...
     where(fldstotmp/=LIS_rc%udef)&
          AC70_struc(n)%ac70(:)%fldsto=fldstotmp/AC70_struc(n)%dt
     ...
     AC70_struc(n)%ac70(:)%fldfrc=fldfrctmp
  endif  

end subroutine ac70_getsws_hymap2

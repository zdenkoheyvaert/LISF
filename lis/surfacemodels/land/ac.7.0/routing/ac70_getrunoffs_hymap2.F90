...
!BOP
! !ROUTINE: ac70_getrunoffs_hymap2
!  \label{ac70_getrunoffs_hymap2}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine ac70_getrunoffs_hymap2(n)
! !USES:
  use ESMF
...
! !DESCRIPTION:
!  
! This routing defines the export variables from Ac70 to HYMAP2
! 
!EOP
...
!if option is not defined, then assume that no evap calculations will be done
  ...
  do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)  !units?
     runoff1(t) = AC70_struc(n)%ac70(t)%runsrf
     runoff2(t) = AC70_struc(n)%ac70(t)%runsub
  enddo
  ...
  if(enable2waycpl==1) then 
    where(sfrunoff/=LIS_rc%udef)sfrunoff=sfrunoff*AC70_struc(n)%dt
    where(baseflow/=LIS_rc%udef)baseflow=baseflow*AC70_struc(n)%dt
  endif
  ...
  if(evapflag.ne.0)then
    ...
    call LIS_verify(status, "ac70_getrunoffs_hymap2: ESMF_StateGet failed for Total Evapotranspiration")

    call ESMF_FieldGet(evapotranspiration_Field,localDE=0,&
         farrayPtr=evapotranspiration,rc=status)
    call LIS_verify(status, "ac70_getrunoffs_hymap2: ESMF_FieldGet failed for Total Evapotranspiration")
    
    do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)  
       evapotranspiration1(t)  =  &
            AC70_struc(n)%ac70(t)%ecan + &
            AC70_struc(n)%ac70(t)%etran + &
            AC70_struc(n)%ac70(t)%edir 
    enddo
...
end subroutine ac70_getrunoffs_hymap2

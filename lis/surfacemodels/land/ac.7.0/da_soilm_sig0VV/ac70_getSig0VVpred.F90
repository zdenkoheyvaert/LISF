...
!BOP
! !ROUTINE: ac70_getSig0VVpred
! \label{ac70_getSig0VVpred}
!
! !REVISION HISTORY:
...
subroutine ac70_getSig0VVpred(n,k,obs_pred)
...
!LIS_histDAtaMod
  call LIS_verify(status, &
       "Error in StateGet in ac70_getSig0VVPred for WCM_Sig0VV")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=s0VV, rc=status)!variable
!s0VV previously defined
  call LIS_verify(status, &
       'Error in FieldGet in ac70_getSig0VVPred for WCM_Sig0VV')
...
end subroutine ac70_getSig0VVpred

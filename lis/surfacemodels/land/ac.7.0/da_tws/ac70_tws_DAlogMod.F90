...
module ac70_tws_DAlogMod
  
  use LIS_constantsMod,  only : LIS_CONST_RHOFW
  use ESMF
! !PUBLIC MEMBER FUNCTIONS:
!------------------------------------------
  public :: ac70_tws_DAlog
!-----------------------------------------
! !PUBLIC TYPES:
!-----------------------------------------
  public :: ACpred_struc
!EOP
  ...
  type, public ::ACpred_dec
     
     real,allocatable ::clmnwater(:,:)
     
  end type ACpred_dec
  
  type (ACpred_dec),allocatable :: ACpred_struc(:)
  
contains 
  
  subroutine ac70_tws_DAlog(n)
    ...
    if(LIS_rc%DAincrMode(n).eq.0) then
       if(LIS_rc%twInterval.eq.2592000.0) then 
          if((LIS_rc%da.eq.1).and.(LIS_rc%hr.eq.12).and.(LIS_rc%mn.eq.0)) then
             if(.not.allocated(ACpred_struc)) then 
                allocate(ACpred_struc(LIS_rc%nnest))
                allocate(ACpred_struc(n)%clmnwater(3,&
                     LIS_rc%npatch(n,LIS_rc%lsm_index)))
             endif
             ACpred_struc(n)%clmnwater = 0.0
          end if
          ...
          if(((LIS_rc%da.eq.5).and.(LIS_rc%hr.eq.4).and.(LIS_rc%mn.eq.0)).or. &
             ...
             ACpred_struc(n)%clmnwater(d,:) = 0.0

             do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
                ACpred_struc(n)%clmnwater(d,t)= &
                     ACpred_struc(n)%clmnwater(d,t) + &
                     AC70_struc(n)%ac70(t)%sneqv   +         &
                     (AC70_struc(n)%ac70(t)%canliq  +         &
                     AC70_struc(n)%ac70(t)%canice) +         &
                     (AC70_struc(n)%ac70(t)%smc(1)  *         &
                     AC70_struc(n)%sldpth(1)*LIS_CONST_RHOFW)        +         &
                     (AC70_struc(n)%ac70(t)%smc(2)  *         &
                     AC70_struc(n)%sldpth(2)*LIS_CONST_RHOFW)        +         &
                     (AC70_struc(n)%ac70(t)%smc(3)  *         &
                     AC70_struc(n)%sldpth(3)*LIS_CONST_RHOFW)        +         &
                     (AC70_struc(n)%ac70(t)%smc(4)  *         &
                     AC70_struc(n)%sldpth(4)*LIS_CONST_RHOFW)        +         &
  ...
  end subroutine ac70_tws_DAlog
  
end module ac70_tws_DAlogMod

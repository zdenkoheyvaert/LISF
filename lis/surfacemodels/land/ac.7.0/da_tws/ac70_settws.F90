...
!
! !INTERFACE:
subroutine ac70_settws(n, LSM_State)
! !USES:
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
! mn
  ...
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 1 failed in ac70_settws")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 2",sm2Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 2 failed in ac70_settws")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 3",sm3Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 3 failed in ac70_settws")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 4",sm4Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 4 failed in ac70_settws")
  call ESMF_StateGet(LSM_State,"Groundwater Storage",gwField,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Groundwater Storage failed in ac70_settws")
  call ESMF_StateGet(LSM_State,"SWE",sweField,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: SWE failed in ac70_settws")
  call ESMF_StateGet(LSM_State,"Snowdepth",snodField,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Snowdepth failed in ac70_settws")


  call ESMF_FieldGet(sm1Field,localDE=0,farrayPtr=soilm1,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 1 failed in ac70_settws")
  call ESMF_FieldGet(sm2Field,localDE=0,farrayPtr=soilm2,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 2 failed in ac70_settws")
  call ESMF_FieldGet(sm3Field,localDE=0,farrayPtr=soilm3,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 3 failed in ac70_settws")
  call ESMF_FieldGet(sm4Field,localDE=0,farrayPtr=soilm4,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 4 failed in ac70_settws")
  call ESMF_FieldGet(gwField,localDE=0,farrayPtr=gws,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Groundwater Storage failed in ac70_settws")
  call ESMF_FieldGet(sweField,localDE=0,farrayPtr=swe,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: SWE failed in ac70_settws")
  call ESMF_FieldGet(snodField,localDE=0,farrayPtr=snod,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Snowdepth failed in ac70_settws")
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     !sm_threshold = AC70_struc(n)%ac70(t)%smcmax - 0.02
     !MAX_THRESHOLD = AC70_struc(n)%ac70(t)%smcmax
     ...
     !Wanshu
     delta5 = gws(t)-AC70_struc(n)%ac70(t)%wa
     ...
     if(soilm1(t).ge.MIN_THRESHOLD .and. soilm1(t).lt. sm_threshold) then
        ...
        write(LIS_logunit,*) 'problem updating sm1 ', soilm1(t), &
             AC70_struc(n)%ac70(t)%smc(1), &
             AC70_struc(n)%ac70(t)%sh2o(1)
     ...
     if(soilm2(t).ge.MIN_THRESHOLD .and. soilm2(t).lt.sm_threshold) then
        ...
        write(LIS_logunit,*) 'problem updating sm2 ', soilm2(t), &
             AC70_struc(n)%ac70(t)%smc(2), &
             AC70_struc(n)%ac70(t)%sh2o(2),WLTSMC(SOILTYP)
     endif
     if(soilm3(t).ge.MIN_THRESHOLD .and. soilm3(t).lt.sm_threshold) then
        ...
        write(LIS_logunit,*) 'problem updating sm3 ', soilm3(t), &
             AC70_struc(n)%ac70(t)%smc(3), &
             AC70_struc(n)%ac70(t)%sh2o(3),WLTSMC(SOILTYP)
     endif
     if(soilm4(t).ge.MIN_THRESHOLD .and. soilm4(t).lt.sm_threshold) then
        ...
        write(LIS_logunit,*) 'problem updating sm4 ', soilm4(t), &
             AC70_struc(n)%ac70(t)%smc(4), &
             AC70_struc(n)%ac70(t)%sh2o(4),WLTSMC(SOILTYP)
     endif
     !     ac70_struc(n)%ac70(t)%wa = gws(t)
     !Wanshu
...
!-----------------------------------------------------------------------------
     ...
     if(update_flag_new(gid)) then
        ...
        do m=1,LIS_rc%nensem(n)
           ...
           if(update_flag_tile(t)) then
              ...
              AC70_struc(n)%ac70(t)%sh2o(2) = &
                   AC70_struc(n)%ac70(t)%sh2o(2) + &
                   delta2
              AC70_struc(n)%ac70(t)%smc(2) = soilm2(t)
              ! MN: Test shutdown the update for 4th layer
              AC70_struc(n)%ac70(t)%sh2o(3) = &
                   AC70_struc(n)%ac70(t)%sh2o(3) + &
                   delta3
              AC70_struc(n)%ac70(t)%smc(3) = soilm3(t)

              AC70_struc(n)%ac70(t)%sh2o(4) = &
                   AC70_struc(n)%ac70(t)%sh2o(4) + &
                   delta4
              AC70_struc(n)%ac70(t)%smc(4) = soilm4(t)

              ! Bailing: add this check for liquid smc
              if(AC70_struc(n)%ac70(t)%sh2o(1)<MIN_THRESHOLD ) &
                   AC70_struc(n)%ac70(t)%sh2o(1) = MIN_THRESHOLD
              if(AC70_struc(n)%ac70(t)%sh2o(1)>MAX_THRESHOLD ) &
                   AC70_struc(n)%ac70(t)%sh2o(1) = MAX_THRESHOLD
              if(AC70_struc(n)%ac70(t)%sh2o(2)<MIN_THRESHOLD ) &
                   AC70_struc(n)%ac70(t)%sh2o(2) = MIN_THRESHOLD
              if(AC70_struc(n)%ac70(t)%sh2o(2)>MAX_THRESHOLD ) &
                   AC70_struc(n)%ac70(t)%sh2o(2) = MAX_THRESHOLD
              if(AC70_struc(n)%ac70(t)%sh2o(3)<MIN_THRESHOLD ) &
                   AC70_struc(n)%ac70(t)%sh2o(3) = MIN_THRESHOLD
              if(AC70_struc(n)%ac70(t)%sh2o(3)>MAX_THRESHOLD ) &
                   AC70_struc(n)%ac70(t)%sh2o(3) = MAX_THRESHOLD
              if(AC70_struc(n)%ac70(t)%sh2o(4)<MIN_THRESHOLD ) &
                   AC70_struc(n)%ac70(t)%sh2o(4) = MIN_THRESHOLD
              if(AC70_struc(n)%ac70(t)%sh2o(4)>MAX_THRESHOLD ) &
                   AC70_struc(n)%ac70(t)%sh2o(4) = MAX_THRESHOLD
              !Wanshu
              if(AC70_struc(n)%ac70(t)%wa+delta5.gt. &
                   MIN_GWS_THRESHOLD.and.&
                   AC70_struc(n)%ac70(t)%wa+delta5.lt. &
              ...
              AC70_struc(n)%ac70(t)%wa = gws_tmp
        ...
        if(LIS_rc%pert_bias_corr.eq.1) then
           ...
           do while(bounds_violation)
              ...
              do m=1,LIS_rc%nensem(n)-1
                 ...
                 if(m.ne.LIS_rc%nensem(n)) then
                    delta(5) = delta(5) + &
                         (AC70_struc(n)%ac70(t)%wa - &
                         AC70_struc(n)%ac70(t_unpert)%wa)
                 endif
              ...
              do m=1,LIS_rc%nensem(n)-1
                 ...
                 tmpval = AC70_struc(n)%ac70(t)%wa - &
                      delta(5)
                 if(tmpval.le.MIN_GWS_THRESHOLD) then
                    AC70_struc(n)%ac70(t)%wa = &
                         max(AC70_struc(n)%ac70(t_unpert)%wa,&
                         MIN_GWS_THRESHOLD)
                    ens_flag(m) = .false.
                 elseif(tmpval.ge.MAX_GWS_THRESHOLD) then
                    AC70_struc(n)%ac70(t)%wa = &
                         min(AC70_struc(n)%ac70(t_unpert)%wa,&
                         MAX_GWS_THRESHOLD)
                    ens_flag(m) = .false.
                 endif
              enddo
              ...
              do m=1,LIS_rc%nensem(n)-1
                 ...
                 if(ens_flag(m)) then
                    ...
                    if(.not.(tmpval.le.MIN_GWS_THRESHOLD .or.&
                       ...
                       AC70_struc(n)%ac70(t)%wa = &
                            AC70_struc(n)%ac70(t)%wa - delta(5)
                       bounds_violation = .false.
                    endif
                 endif
              ...
              if(nIter.gt.10.and.bounds_violation) then
                 ...
                 do j=1,4
                    do m=1,LIS_rc%nensem(n)
                       ...
                       if(AC70_struc(n)%ac70(t)%sh2o(j).gt. &
                            MAX_THRESHOLD.or.&
                            AC70_struc(n)%ac70(t)%smc(j).gt. &
                            MAX_THRESHOLD) then
                          AC70_struc(n)%ac70(t)%sh2o(j) = MAX_THRESHOLD
                          AC70_struc(n)%ac70(t)%smc(j) = MAX_THRESHOLD
                       endif

                       if(AC70_struc(n)%ac70(t)%sh2o(j).lt. &
                            MIN_THRESHOLD.or.&
                            AC70_struc(n)%ac70(t)%smc(j).lt. &
                            MIN_THRESHOLD) then
                 ...
                 do m=1,LIS_rc%nensem(n)
                    ...
                    if(AC70_struc(n)%ac70(t)%wa.gt. &
                         MAX_GWS_THRESHOLD) then
                       AC70_struc(n)%ac70(t)%wa = MAX_GWS_THRESHOLD
                    endif

                    if(AC70_struc(n)%ac70(t)%wa.lt. &
                         MIN_GWS_THRESHOLD) then
                       AC70_struc(n)%ac70(t)%wa = MIN_GWS_THRESHOLD
                    endif
                 enddo
#endif
                 do m=1,LIS_rc%nensem(n)-1
                    ...
                    if(.not.(AC70_struc(n)%ac70(t_unpert)%wa.le. &
                         MIN_GWS_THRESHOLD .or.&
                         AC70_struc(n)%ac70(t_unpert)%wa.gt. &
                         (MAX_GWS_THRESHOLD))) then
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     swe_old = ac70_struc(n)%ac70(t)%sneqv
     snowh_old = ac70_struc(n)%ac70(t)%snowh

     dsneqv = swe(t)*1000.0 - ac70_struc(n)%ac70(t)%sneqv !in mm
     dsnowh = snod(t) - ac70_struc(n)%ac70(t)%snowh  !in m

!alternate option
#if 0
!only update dshowh, update SWE based on density
     dsnowh = snod(t) - ac70_struc(n)%ac70(t)%snowh  !in m
     snow_dens = ac70_struc(n)%ac70(t)%sneqv/&
          ac70_struc(n)%ac70(t)%snowh
     swe_new = snow_dens*snod(t)

     dsneqv = swe_new - ac70_struc(n)%ac70(t)%sneqv !in mm

#endif
     call ac70_snow_update(n, t, dsneqv, dsnowh)

     if(ac70_struc(n)%ac70(t)%sneqv.lt.0.or.&
          ac70_struc(n)%ac70(t)%snowh.lt.0) then
     ...
     if(ac70_struc(n)%ac70(t)%sneqv.gt.0.and.&
          ac70_struc(n)%ac70(t)%snowh.le.0) then
        write(LIS_logunit,*) &
             '[WARN] state update leads to positive SWE but zero snow depth'
        write(LIS_logunit,*) &
             '[WARN] ',ac70_struc(n)%ac70(t)%sneqv,&
             ac70_struc(n)%ac70(t)%snowh
        write(LIS_logunit,*) '[WARN] rejecting the update ...'
        ac70_struc(n)%ac70(t)%sneqv = swe_old
        ac70_struc(n)%ac70(t)%snowh = snowh_old
     endif

     if(ac70_struc(n)%ac70(t)%sneqv.le.0.and.&
          ac70_struc(n)%ac70(t)%snowh.gt.0) then
        write(LIS_logunit,*) &
             '[WARN] state update leads to positive snow depth but zero SWE'
        write(LIS_logunit,*) '[WARN] ',ac70_struc(n)%ac70(t)%sneqv,&
             ac70_struc(n)%ac70(t)%snowh
        write(LIS_logunit,*) '[WARN] rejecting the update ...'

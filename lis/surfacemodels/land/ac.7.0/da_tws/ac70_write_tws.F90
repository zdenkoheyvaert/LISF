...
!BOP
! !ROUTINE: ac70_write_tws
! \label{ac70_write_tws}
!
! !REVISION HISTORY:
! 14 Mar 2017: Sujay Kumar; Initial Specification
!
! !INTERFACE:
subroutine ac70_write_tws(ftn,n, LSM_State)
...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     tmp(t) = AC70_struc(n)%ac70(t)%smc(1)
  enddo  
  call LIS_writevar_restart(ftn,n,1,tmp)

  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     tmp(t) = AC70_struc(n)%ac70(t)%smc(2)
  enddo
  call LIS_writevar_restart(ftn,n,1,tmp)

  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     tmp(t) = AC70_struc(n)%ac70(t)%smc(3)
  enddo
  call LIS_writevar_restart(ftn,n,1,tmp)
  ...
  !Wanshu
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     tmp(t) = AC70_struc(n)%ac70(t)%wa
  enddo
  call LIS_writevar_restart(ftn,n,1,tmp)
  !---------
  deallocate(tmp)

end subroutine ac70_write_tws

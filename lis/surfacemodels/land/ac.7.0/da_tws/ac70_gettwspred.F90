...
!BOP
! !ROUTINE: ac70_gettwspred
! \label{ac70_gettwspred}
!
! !REVISION HISTORY:
! 14 Mar 2017: Sujay Kumar; Initial Specification
!
! !INTERFACE:
subroutine ac70_gettwspred(n, k,obs_pred)
! !USES:
  use ESMF
  use LIS_coreMod
  use LIS_logMod
  use LIS_DAobservationsMod
  use ac70_tws_DAlogMod
  use ac70_lsmMod
...
!EOP
  ...
  do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)
     tws(t) = (ACpred_struc(n)%clmnwater(1,t) + &	
          ACpred_struc(n)%clmnwater(2,t)+&
          ACpred_struc(n)%clmnwater(3,t))/3.	
  enddo
  call LIS_convertPatchSpaceToObsEnsSpace(n,k,&
       LIS_rc%lsm_index, &
       tws,&
       obs_pred)
end subroutine ac70_gettwspred

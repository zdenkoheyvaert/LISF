...
!BOP
! !ROUTINE: ac70_descale_tws
! \label{ac70_descale_tws}
!
! !REVISION HISTORY:
! 14 Mar 2017: Sujay Kumar; Initial Specification
!
! !INTERFACE:
subroutine ac70_descale_tws(n, LSM_State, LSM_Incr_State)
...
! Natt
  ...
  call ESMF_StateGet(LSM_Incr_State, "Groundwater Storage",gwIncrField, &
       rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Groundwater Storage failed in ac70_descale")
  ...
  call LIS_verify(status, &
       'ESMF_StateGet failed for sm1 in ac70_descale_tws')
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 2",sm2Field,rc=status)
  call LIS_verify(status, &
       'ESMF_StateGet failed for sm2 in ac70_descale_tws')
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 3",sm3Field,rc=status)
  call LIS_verify(status, &
       'ESMF_StateGet failed for sm3 in ac70_descale_tws')
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 4",sm4Field,rc=status)
  call LIS_verify(status, &
       'ESMF_StateGet failed for sm4 in ac70_descale_tws')
  call ESMF_StateGet(LSM_State,"SWE",sweField,rc=status)
  call LIS_verify(status, &
       'ESMF_StateGet failed for SWE in ac70_descale_tws')
  call ESMF_StateGet(LSM_State,"Snowdepth",snodField,rc=status)
  call LIS_verify(status, &
       'ESMF_StateGet failed for Snowdepth in ac70_descale_tws')

  call ESMF_FieldGet(sm1Field,localDE=0,farrayPtr=soilm1,rc=status)
  call LIS_verify(status, &
       'ESMF_FieldGet failed for sm1 in ac70_descale_tws')
  call ESMF_FieldGet(sm2Field,localDE=0,farrayPtr=soilm2,rc=status)
  call LIS_verify(status, &
       'ESMF_FieldGet failed for sm2 in ac70_descale_tws')
  call ESMF_FieldGet(sm3Field,localDE=0,farrayPtr=soilm3,rc=status)
  call LIS_verify(status, &
       'ESMF_FieldGet failed for sm3 in ac70_descale_tws')
  call ESMF_FieldGet(sm4Field,localDE=0,farrayPtr=soilm4,rc=status)
  call LIS_verify(status, &
       'ESMF_FieldGet failed for sm4 in ac70_descale_tws')
  call ESMF_FieldGet(sweField,localDE=0,farrayPtr=swe,rc=status)
  call LIS_verify(status, &
       'ESMF_FieldGet failed for SWE in ac70_descale_tws')
  call ESMF_FieldGet(snodField,localDE=0,farrayPtr=snod,rc=status)
  call LIS_verify(status, &
       'ESMF_FieldGet failed for Snowdepth in ac70_descale_tws')
  ...
  call ESMF_StateGet(LSM_Incr_State,"Soil Moisture Layer 1",sm1IncrField, &
       rc=status)
  call LIS_verify(status,&
       "ESMF_StateGet: Soil Moisture Layer 1 failed in ac70_descale_tws")
  call ESMF_StateGet(LSM_Incr_State,"Soil Moisture Layer 2",sm2IncrField, &
       rc=status)
  call LIS_verify(status,&
       "ESMF_StateGet: Soil Moisture Layer 2 failed in ac70_descale_tws")
  call ESMF_StateGet(LSM_Incr_State,"Soil Moisture Layer 3",sm3IncrField, &
       rc=status)
  call LIS_verify(status,&
       "ESMF_StateGet: Soil Moisture Layer 3 failed in ac70_descale_tws")
  call ESMF_StateGet(LSM_Incr_State,"Soil Moisture Layer 4",sm4IncrField, &
       rc=status)
  call LIS_verify(status,&
       "ESMF_StateGet: Soil Moisture Layer 4 failed in ac70_descale_tws")
  ...
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 1 failed in ac70_descale_tws")
  call ESMF_FieldGet(sm2IncrField,localDE=0,farrayPtr=soilmIncr2,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 2 failed in ac70_descale_tws")
  call ESMF_FieldGet(sm3IncrField,localDE=0,farrayPtr=soilmIncr3,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 3 failed in ac70_descale_tws")
  call ESMF_FieldGet(sm4IncrField,localDE=0,farrayPtr=soilmIncr4,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 4 failed in ac70_descale_tws")
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     soilm1(t) = soilm1(t) / &
          (AC70_struc(n)%sldpth(1)*LIS_CONST_RHOFW) ! mm -> m3m-3
     soilm2(t) = soilm2(t) / &
          (AC70_struc(n)%sldpth(2)*LIS_CONST_RHOFW) ! mm -> m3m-3
     soilm3(t) = soilm3(t) / &
          (AC70_struc(n)%sldpth(3)*LIS_CONST_RHOFW) ! mm -> m3m-3
     soilm4(t) = soilm4(t) / &
          (AC70_struc(n)%sldpth(4)*LIS_CONST_RHOFW) ! mm -> m3m-3
...
end subroutine ac70_descale_tws

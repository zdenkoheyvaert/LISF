...
!
! !DESCRIPTION:
!  This routine transfers the LIS provided forcing into the Ac70
!  model tiles.
!
!  The arguments are:
!  \begin{description}
...
!EOP
    ...
    call LIS_verify(status, "Ac70_f2t: error getting Tair")

    call ESMF_FieldGet(tmpField, localDE = 0, farrayPtr = tmp, rc = status)
    call LIS_verify(status, "Ac70_f2t: error retrieving Tair")
    
    ! get near surface specific humidity
    call ESMF_StateGet(LIS_FORC_State(n), trim(LIS_FORC_Qair%varname(1)), q2Field, rc=status)
    call LIS_verify(status, "Ac70_f2t: error getting Qair")

    call ESMF_FieldGet(q2Field, localDE = 0, farrayPtr = q2, rc = status)
    call LIS_verify(status, "Ac70_f2t: error retrieving Qair")
    
    ! get incident shortwave radiation
    call ESMF_StateGet(LIS_FORC_State(n), trim(LIS_FORC_SWdown%varname(1)), swdField, rc=status)
    call LIS_verify(status, "Ac70_f2t: error getting Swdown")

    call ESMF_FieldGet(swdField, localDE = 0, farrayPtr = swd, rc = status)
    call LIS_verify(status, "Ac70_f2t: error retrieving Swdown")
    
    ! get incident longwave radiation
    call ESMF_StateGet(LIS_FORC_State(n), trim(LIS_FORC_LWdown%varname(1)), lwdField, rc=status)
    call LIS_verify(status, "Ac70_f2t: error getting Lwdown")

    call ESMF_FieldGet(lwdField, localDE = 0, farrayPtr = lwd, rc = status)
    call LIS_verify(status, "Ac70_f2t: error retrieving Lwdown")
    
    ! get eastward wind
    call ESMF_StateGet(LIS_FORC_State(n), trim(LIS_FORC_Wind_E%varname(1)), uField, rc=status)
    call LIS_verify(status, "Ac70_f2t: error getting Wind_E")

    call ESMF_FieldGet(uField, localDE = 0, farrayPtr = uwind, rc = status)
    call LIS_verify(status, "Ac70_f2t: error retrieving Wind_E")
    
    ! get northward wind
    call ESMF_StateGet(LIS_FORC_State(n), trim(LIS_FORC_Wind_N%varname(1)), vField, rc=status)
    call LIS_verify(status, "Ac70_f2t: error getting Wind_N")

    call ESMF_FieldGet(vField, localDE = 0, farrayPtr = vwind, rc = status)
    call LIS_verify(status, "Ac70_f2t: error retrieving Wind_N")
    
    ! get surface pressure
    call ESMF_StateGet(LIS_FORC_State(n), trim(LIS_FORC_Psurf%varname(1)), psurfField, rc=status)
    call LIS_verify(status, "Ac70_f2t: error getting Psurf")

    call ESMF_FieldGet(psurfField, localDE = 0, farrayPtr = psurf, rc = status)
    call LIS_verify(status, "Ac70_f2t: error retrieving Psurf")
    
    ! get rainfall rate
    call ESMF_StateGet(LIS_FORC_State(n), trim(LIS_FORC_Rainf%varname(1)), pcpField, rc=status)
    call LIS_verify(status, "Ac70_f2t: error getting Rainf")

    call ESMF_FieldGet(pcpField, localDE = 0, farrayPtr = pcp, rc = status)
    call LIS_verify(status, "Ac70_f2t: error retrieving Rainf")
    ...
    if(LIS_Forc_Snowf%selectOpt .eq. 1) then 
        ...
        call LIS_verify(status, "Ac70_f2t: error getting Snowf")
    ...
    do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
        ...
        ! TAIR
        AC70_struc(n)%ac70(t)%tair = AC70_struc(n)%ac70(t)%tair + tmp(tid)

        ! QAIR
        AC70_struc(n)%ac70(t)%qair = AC70_struc(n)%ac70(t)%qair + q2(tid)

        ! SWDOWN
        AC70_struc(n)%ac70(t)%swdown = AC70_struc(n)%ac70(t)%swdown + swd(tid)

        ! LWDOWN
        AC70_struc(n)%ac70(t)%lwdown = AC70_struc(n)%ac70(t)%lwdown + lwd(tid)

        ! WIND_E
        AC70_struc(n)%ac70(t)%wind_e = AC70_struc(n)%ac70(t)%wind_e + uwind(tid)

        ! WIND_N
        AC70_struc(n)%ac70(t)%wind_n = AC70_struc(n)%ac70(t)%wind_n + vwind(tid)

        ! PSURF
        AC70_struc(n)%ac70(t)%psurf = AC70_struc(n)%ac70(t)%psurf + psurf(tid)

        ! RAINF
        if(pcp(tid).ne.LIS_rc%udef) then
           AC70_struc(n)%ac70(t)%prcp = AC70_struc(n)%ac70(t)%prcp + pcp(tid)
        endif

        ! SNOWF
        ! If there is snowf add it to precipitation.  Ac70 does not use

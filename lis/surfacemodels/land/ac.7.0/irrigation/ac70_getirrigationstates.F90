...
! !USES:
  ...
  use LIS_logMod
  use Ac70_lsmMod
  use MODULE_SF_ACLSM_70, only: MAXSMC, REFSMC, WLTSMC
  use LIS_vegDataMod, only: LIS_read_shdmin, LIS_read_shdmax
! use MODULE_SF_ACLSM_70, only: SMCMAX, PSISAT, DKSAT, BEXP
...
! Aug 2016: Wanshu Nie; Incorporated into Ac
...
!----------------------------------------------------------------------
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     smcmax = MAXSMC(AC70_struc(n)%ac70(t)%soiltype) 
     smcref = REFSMC(AC70_struc(n)%ac70(t)%soiltype)
     smcwlt = WLTSMC(AC70_struc(n)%ac70(t)%soiltype)
     sfctemp = AC70_struc(n)%ac70(t)%sfctmp
     ...
     shdfac =  AC70_struc(n)%ac70(t)%shdfac_monthly(LIS_rc%mo)
...
!                if(ippix.gt.0.0) then  ! irrigated tile
                 ...
                 !shdmin = minval(AC70_struc(n)%ac70(t)%shdfac_monthly)
                 !shdmax = maxval(AC70_struc(n)%ac70(t)%shdfac_monthly)
                ...
                !!!!! SPRINKLER IRRIGATION
                    if(LIS_rc%irrigation_type.eq."Sprinkler") then
                       ...
                       if(ltime.eq.shift_otimes) then !Check moisture availability at otimes only
                         ...
                         !-------------------------------------------------------------
                          if(lroot.gt.0) then 
                             do k=1,lroot
                                asmc = asmc + AC70_struc(n)%ac70(t)%smc(k)*&
                                     rdpth(k)*1000.0
                         ...
                         !--------------------------------------------------------------- 
                             ...
                             if(ma.le.LIS_rc%irrigation_thresh) then 
                                do k=1,lroot
                                   water(k) = &
                                        (smcref-AC70_struc(n)%ac70(t)%smc(k))*&
                                        rdpth(k)*1000.0
                  ...
                  elseif(LIS_rc%irrigation_type.eq."Drip") then
                     ...
                     twater = 0.0
                         ...
                         AC70_struc(n)%ac70(t)%smc(1) + (twater-twater2)/(sldpth(1)*1000.0) !! check this with Sujay
...
!                            ma = (asmc-tsmcwlt) /(tsmcref - tsmcwlt)   ! Original
                            ...
                            if( ma .le. LIS_rc%irrigation_thresh ) then
                              do l = 1, LIS_rc%irrigation_mxsoildpth
                                 if( l == 1 ) then
                                   twater = (SMCMAX - AC70_struc(n)%ac70(t)%smc(l))*sldpth(l)*1000.0
                                 else
                                 ! BZ modification 4/2/2015 to saturate entire column and apply ippix 
                                   twater = twater + (smcmax - AC70_struc(n)%ac70(t)%smc(l))*sldpth(l)*1000.0
...
!                              noah33_struc(n)%noah(t)%smc(1) = smcmax   ! Original
                            ...
                            ! BZ modification 4/2/2015 to account for ippix and all soil layers:
                               do l = 1, LIS_rc%irrigation_mxsoildpth
                                  AC70_struc(n)%ac70(t)%smc(l) = IrrigScale(t)*smcmax + &
                                                 (1-IrrigScale(t))*AC70_struc(n)%ac70(t)%smc(l)
                               end do
...
!                                             (1-IrrigScale(t))*noah33_struc(n)%noah(t)%smc(4)
           ...
           if (LIS_rc%irrigation_GWabstraction.eq.1) then
              AWS = AC70_struc(n)%ac70(t)%wa
              Dtime = AC70_struc(n)%dt
              AC70_struc(n)%ac70(t)%wa = AWS - irrigRate(t)*Dtime
           end if
       end if

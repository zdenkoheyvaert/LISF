...
!BOP
! !ROUTINE: ac70_getsnwdpred
! \label{ac70_getsnwdpred}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine ac70_getsnwdpred_TB(n, k, obs_pred)
...
!EMK...Disabled this subroutine.  The call to ac70_LIS_SVM is not passing
!the expected arguments, so the code is badly broken.  The task of fixing this
!rests with the original developer.
#if 1

  write(LIS_logunit,*)'[ERROR] ac70_getsnwdpred_TB is currently disabled.'
  write(LIS_logunit,*)'The subroutine call to ac70_LIS_SVM is broken'
  write(LIS_logunit,*)' (wrong arguments are passed)'
  write(LIS_logunit,*)'Edit ac70_getsnwdpred_TB.F90 to re-enable.'
  call LIS_endrun()

#else

  call ac70_LIS_SVM(n)     !kyh20170823


  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     !snwd(t) = ac70_struc(n)%ac70(t)%snowh*1000.0 !obs in mm   !original
!svk commenting out
!     TB_18V(t) = ac70_struc(n)%ac70(t)%TB_18V_lis   !K          !kyh20171003
!     TB_18H(t) = ac70_struc(n)%ac70(t)%TB_18H_lis   !K          !kyh20171003
!     TB_36V(t) = ac70_struc(n)%ac70(t)%TB_36V_lis   !K          !kyh20171003
!     TB_36H(t) = ac70_struc(n)%ac70(t)%TB_36H_lis   !K          !kyh20171003
  enddo
...
end subroutine ac70_getsnwdpred_TB

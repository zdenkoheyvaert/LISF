...
!BOP
! !ROUTINE: ac70_getsmpred
! \label{ac70_getsmpred}
!
! !REVISION HISTORY:
...
!rescaling to relative wetness space. 
     if(LIS_rc%dascaloption(k).eq."Linear scaling") then 
        ...
        if(ac70_dasm_struc(n)%ntimes.gt.1) then 
           kk = LIS_rc%mo
        else
           kk = 1
        endif
        ...
        do i=1,LIS_rc%obs_ngrid(k)
           do m=1,LIS_rc%nensem(n)
              obs_tmp = (obs_pred(i,m) - ac70_dasm_struc(n)%model_xrange(i,kk,1))/&
                   (ac70_dasm_struc(n)%model_xrange(i,kk,ac70_dasm_struc(n)%nbins) - & 
                   ac70_dasm_struc(n)%model_xrange(i,kk,1))
     ...
     do i=1,LIS_rc%npatch(n,LIS_rc%lsm_index),LIS_rc%nensem(n)
        do m=1,LIS_rc%nensem(n)
           ...
           inputs_tp(1) = AC70_struc(n)%ac70(t)%smc(1)
           inputs_tp(2) = AC70_struc(n)%ac70(t)%prcp*(1-AC70_struc(n)%ac70(t)%fpice) !Noah33 rainf
           inputs_tp(3) = AC70_struc(n)%ac70(t)%prcp*AC70_struc(n)%ac70(t)%fpice     !Noah33 snowf    
           !MN: NOTE:  Ac70 --> prcp (total precip), fpice (snow fraction in precipitation [-])
           inputs_tp(4) = AC70_struc(n)%ac70(t)%fveg !Noah33 shdfac
           inputs_tp(5) = AC70_struc(n)%ac70(t)%sstc(AC70_struc(n)%nsnow+1)  !Noah33 stc(1)
           inputs_tp(6) = AC70_struc(n)%ac70(t)%sneqv*LIS_CONST_RHOFW

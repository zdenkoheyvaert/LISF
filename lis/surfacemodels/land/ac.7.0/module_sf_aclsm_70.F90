...
! 4. irrigation
  ...
  ! SY: Begin moving to here from AC_VEG_PARAMETERS_70 to facilitate OPTUE
  ...
  ! SY: End moving to here from AC_VEG_PARAMETERS_70 to facilitate OPTUE

!------------------------------------------------------------------------------------------!
END MODULE AC_GLOBALS_70
!------------------------------------------------------------------------------------------!
!------------------------------------------------------------------------------------------!
MODULE AC_VEG_PARAMETERS_70

    USE AC_GLOBALS_70, ONLY: MVT, MBAND,  &
    ...
    ! SY: Begin commenting out and moving to AC_GLOBALS_70 and using from there to facilitate OPTUE
!    INTEGER, PARAMETER :: MVT   = 27
!    INTEGER, PARAMETER :: MBAND = 2
    ! SY: End commenting out and moving to AC_GLOBALS_70 and using from there to facilitate OPTUE
    ...
    ! SY: Begin commenting out some variables and moving to AC_GLOBALS_70 and using from there to facilitate OPTUE
!    REAL :: CH2OP(MVT)       !maximum intercepted h2o per unit lai+sai (mm)
!    REAL :: DLEAF(MVT)       !characteristic leaf dimension (m)
!    REAL :: Z0MVT(MVT)       !momentum roughness length (m)
!    REAL :: HVT(MVT)         !top of canopy (m)
...
!    REAL :: WDPOOL(MVT)      !wood pool (switch 1 or 0) depending on woody or not [-]
!    REAL :: TDLEF(MVT)       !characteristic T for leaf freezing [K]
    ! SY: End commenting out some variables and moving to AC_GLOBALS_70 and using from there to facilitate OPTUE
...
END MODULE AC_VEG_PARAMETERS_70
! ==================================================================================================
! ==================================================================================================
MODULE AC_RAD_PARAMETERS_70
...
END MODULE AC_RAD_PARAMETERS_70
! ==================================================================================================

MODULE AC_ROUTINES_70
  USE AC_GLOBALS_70
  IMPLICIT NONE
...
!   WRITE(*,*) "ERRENG =",ERRENG
   IF(ABS(ERRENG) > 0.01) THEN
      ...
      call wrf_message(trim(message))
      call wrf_error_fatal("Energy budget problem in AC LSM")
   END IF
...
! canopy height
        ...
        IF((HCAN-ZPD) <= 0.) THEN
          ...
          call wrf_message ( message )
          call wrf_error_fatal ( "CRITICAL PROBLEM IN MODULE_SF_ACLSM:VEGEFLUX" )
        END IF
...
!    QINSUR = PONDING/DT * 0.001
    ...
    if(fldfrc==1)then
      QINSUR = QINSUR + (rivsto + fldsto) !surface water storage units are in m/s (See HYMAP2_routing_run.F90 and ac70_getsws_hymap2.F90)
    endif
...
! lake/soil water balances
    ...
    if(fldfrc==0)then
      RUNSRF = RUNSRF + (rivsto + fldsto)*1000. !surface water storage units are in m/s (See HYMAP2_routing_run.F90 and ac70_getsws_hymap2.F90)
    endif
...
! ------------------------------------------------------------------------------------------
  SUBROUTINE BVOCFLUX(VOCFLX,  VEGTYP,  VEGFRAC,  APAR,   TV )
    use AC_VEG_PARAMETERS_70 , ONLY : SLAREA, EPS
! ------------------------------------------------------------------------------------------
...
! ==================================================================================================
  ...
  subroutine ac_options(idveg     ,iopt_crs  ,iopt_btr  ,iopt_run  ,iopt_sfc  ,iopt_frz , & 
...
! -------------------------------------------------------------------------------------------------
  ...
  end subroutine ac_options
 
END MODULE AC_ROUTINES_70
! ==================================================================================================

MODULE MODULE_SF_ACLSM_70

  USE AC_ROUTINES_70
  USE AC_GLOBALS_70
  USE AC_VEG_PARAMETERS_70

END MODULE MODULE_SF_ACLSM_70

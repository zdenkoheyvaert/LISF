...
#include "LIS_misc.h"
!BOP
! !ROUTINE: ac70_sfc2cmem3
! \label{ac70_sfc2cmem3}
!
! !REVISION HISTORY:
!  37 Mar 2009: Sujay Kumar; Initial Code
!  2  May 2011: Yudong Tian; Modified for Noah 3.2 
!  22 Jun 2018: Peter Shellito; Modified for Ac70. 
!
! !INTERFACE:
subroutine ac70_sfc2cmem3(n, sfcState)
! !USES:      
  use ESMF
...
! pvar is the variable dimensioned in the patch space
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     lsm_var(t) = sqrt(AC70_struc(n)%ac70(t)%wind_e*& 
          AC70_struc(n)%ac70(t)%wind_e+& 
          AC70_struc(n)%ac70(t)%wind_n*&
          AC70_struc(n)%ac70(t)%wind_n)
  enddo
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     lsm_var(t) = AC70_struc(n)%ac70(t)%snowh 
  enddo
  call LIS_patch2tile(n,LIS_rc%lsm_index,snow_depth,lsm_var)
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     if(AC70_struc(n)%ac70(t)%sneqv.gt.0)  then 
        ...
        lsm_var(t) = AC70_struc(n)%ac70(t)%sneqv &
             / AC70_struc(n)%ac70(t)%snowh / 1000.0
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     if (AC70_struc(n)%ac70(t)%snowh  .gt. 0.0001) then  
        snow_cov(t)=1.0
        land_cov(t)=0.0
     else
        snow_cov(t)=0.0
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     lsm_var(t)     = AC70_struc(n)%ac70(t)%vegetype 
  enddo
  call LIS_patch2tile(n,LIS_rc%lsm_index,land_type, lsm_var)
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     lsm_var(t) = AC70_struc(n)%ac70(t)%trad
  enddo
